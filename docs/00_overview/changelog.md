...
- **P0 修正（讀者看得到的錯）**：CDR jitter **tolerance** 誤標為 $\lvert H_{lp}\rvert^2$（那是 transfer；tolerance 由 $1-H_{lp}=H_{hp}$ 決定，新增 JTOL 小節與 worked numbers）；isf_definition 對 [P3] Eq.(30) 的符號說法與 paper_003 相反（原文為 **＋**，[P4] Eq.(26) $\tilde\Gamma_1\angle90°$ 與本站 $-\sin\theta$ 同號，已統一）；effective_isf 同一情境算出 0.22 與 0.31 兩個 $\Gamma_{rms}^{eff}$（統一為 0.31／$-152.2$ dBc/Hz）；期末題 7 輸入題目給的 $-155$ 也判對且頁尾顯示「±0%」（改問 1 MHz：$-151.47$ dBc/Hz，NumericQuiz 顯示 ±0.1%）；**[P4] APF 符號以渲染頁為準**：$\tilde\Lambda(\varphi)$＝amplitude ISF（1/C，Eq.(18)/(24)）、$\Delta(\varphi)=\tau_0\tilde\Lambda$＝APF（1/A，Eq.(19)/(25)）、$\Delta_1=(\tau_0/q_{max})\angle0°$（Eq.(26)），全站 16 檔統一（計劃裡的 Λ 是 raw text 希臘字母缺字造成的誤讀）；首頁「9 步」→「12 步」；**[P1] 式號頁碼**：Eq.(1)–(5) p.180、Eq.(6)–(9) p.181、Eq.(10)–(11) p.182、Eq.(12)–(18) p.183（27 檔 44 處，逐頁渲染核對）；lab_24 notebook 重生（補 Part 5）；上方 [P2] Eq.(12)／Eq.(6) 兩處歸屬更正。
- **論文未教段落補齊（fable）**：[P1] Sec.III-F Eq.(28)–(29) 收斂回 Leeson 型式與 Craninckx–Steyaert 2× 差（derivation_leeson Step 6）；[P2] Eq.(27)–(30) 短通道模型＋Sec.VIII 實測驗證表（Table I–III，#3/#12 重算）；[P4] Sec.VII ISF shaping for division（半波對稱殺 $c_2$、Table IV、新圖 isf_shaping_division）；[P3] 深讀頁補矽驗證（Fig.12–16）、Adler 中間代數、Eq.(6)–(10)、footnote 3。
- **系統層新頁**：[adpll_tdc_dco](/06_design_insights/adpll_tdc_dco)（TDC／DCO 量化雜訊床與預算式）、[cdr_bang_bang_jtol](/06_design_insights/cdr_bang_bang_jtol)（$K_{bb}$、JTOL mask、SSC）、[design_recipe](/06_design_insights/design_recipe)（規格 → FOM 可行性 → LC/ring → Q/C/L/Rp → $q_{max}$ → $\mathcal{L}(\Delta f)$ → jitter，一條 worked 走到底）；charge-pump type-II 設計式（lab_13 §2＋`pll_utils.design_type2`）與 PLL jitter–power FOM；期末考補熱身題 0a/0b＋每題學習路徑標籤；lab_42（coupled QVCO，重現 1.9°）；lab_43（class-C/D/F 的 ISF 波形工程）。
- **動線與門面**：學習路徑補回 9 頁孤立先備頁（tank_Q、tank_swing、waveform_slope、device_noise_mapping、stochastic_noise_basics、dsp_view、pll_noise_budget、interactive_calculator、design_recipe）；glossary 新增「注入鎖定與量測」19 詞；notation.md 新增注入鎖定／PLL／Allan／SerDes 符號家族並消歧 α／β／Δω／ω_c／τ₀／ζ／N／M（各頁首次使用加一句本站慣例）；各章側欄 landing page；python_environment／README 環境描述對齊實際檔案；麵包屑一致化（9 個 03 章頁＋4 個章尾頁）；[p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation) 與 [derivation_autocorrelation_wiener_khinchin](/99_appendix/derivation_autocorrelation_wiener_khinchin) 兩個附錄頁（jitter_kernels 與 white_noise_to_phase_noise 瘦身）；reciprocal mixing／EVM 小節；量測頁 datasheet 分段積分（12 kHz–20 MHz：8.45 ps）＋ cross-correlation 三陷阱；DLL 規則 5 與 PI 量化；PAM4 28/56 GBd 與 12-bit 1 GS/s ADC 例題；選型決策表；7 頁測驗答案外洩改題、4 個 widget 宿主頁補錨定預設值的測驗；theory_map 重建（含 v10 注入群與 v11 新頁）；figure_index 60 圖零孤兒（原漏 6 張）；references.md 外部文獻編號自洽；robots.txt。
- **英文版**：102 頁結構平價 0 差異；47 支含中文軸標的圖在 EN 頁下加 76 處 Translator's note（新工具 `scripts/list_cjk_figures.py`）；`\text{}` 內殘留中文清除。
- **新工具**：`scripts/validate_mdx.mjs`（Docusaurus 同款 MDX＋GFM＋math 編譯檢查，含 `{#id}` 錨點）、`scripts/check_en_parity.py`（12 項結構指標）。
- **過程誠實備註**：兩度撞到用量上限中斷（wave 2 由中斷狀態接續完成，未重做）；wave 2 的審查改用 sonnet 以省用量，數字由 gate 全數重驗；R11-05 實作者偏離計劃改用 Δ 是對的（站主親自渲染頁面確認）；κ Eq.(12) 是本站**第四次**「蓋了已核實章的錯」——教訓：「已核實」標籤也要重驗，raw text 的希臘字母是空白。
- 規模：**102 頁×2 語系、60 圖、52 模擬、21 互動元件、211 可驗證例題（0 錯）**。

- **[P2] Eq.(12), p.793 印刷式含 $\omega_0$**：$\kappa=\dfrac{\Gamma_{rms}}{q_{max}\,\omega_0}\sqrt{\tfrac12\,\overline{i_n^2}/\Delta f}$，單位 $\sqrt{\text{s}}$，是 Eq.(8) $\sigma_{\Delta T}=\kappa\sqrt{\Delta T}$ 的**時間版**常數（由 Eq.(10) $\sigma_{\Delta\phi}=\omega_0\sigma_{\Delta T}$ 與 Eq.(11) 的相位方差合成）。本站的相位版 $\kappa_\phi=(\Gamma_{rms}/q_{max})\sqrt{S_i/2}$（rad/√s；canonical $\kappa_\phi^2=0.125\ \text{rad}^2/\text{s}$、$D=\kappa_\phi^2/2$）物理正確、**全站數字一個都沒變**，錯的只有歸屬：v4–v5 曾把「Eq.(12) 無 $\omega_0$」當作已核實（v4 甚至把稽核的正確指正當成「假修正」擋下），並在 diffusion_dictionary 寫成論文的「單位陷阱」——那是本站的誤讀，論文沒錯。v11 站主重看渲染 PDF 後更正 paper_002、diffusion_dictionary、beyond_lorentzian、lc_vs_ring、jitter_kernels、equation_index、figure_index、tank_swing、math_identities、references、derivation_floquet_ppv（zh＋EN），關係式統一為 $\kappa_\phi=\omega_0\kappa_t$、$\kappa_t=\kappa_\phi/(2\pi f_0)$（canonical $1.125\times10^{-11}\ \sqrt{\text{s}}$）。
- **[P2] Eq.(6), p.792 是 $/2$ 家族**：印刷式 $\mathcal{L}\{f_{off}\}=\Gamma_{rms}^2/(8\pi^2f_{off}^2)\cdot(\overline{i_n^2}/\Delta f)/q_{max}^2$，分母 $8\pi^2f^2=2\Delta\omega^2$，比 [P1] Eq.(21) 的 $/4$ SSB 家族（本站 $-148$ dBc/Hz）**高 3 dB**（$-145$）。paper_002 差動 ring 數值例原本寫成「與 [P1] 的 4 同族、時域 /2 再 +3 dB」——方向相反，v11 更正：Eq.(34) 由 $2N\times$Eq.(6) 收成，$-82.7$／$-78.0$ dBc/Hz 是 $/2$ 家族值（$/4$ 家族為 $-85.7$／$-81.0$），$\Delta\mathcal{L}=+4.77$ dB 不受影響（lc_vs_ring 例 3 的同一段慣例 flag 同步更正）。
- **順手更正一處引用**：math_identities 把 $\sigma_{\Delta t}=\kappa\sqrt{\Delta t}$ 標成「[P2] Eq.(10), p.793」——渲染 PDF 上那是 Eq.(8), p.792；Eq.(10), p.793 是相位→時間換算 $\sigma_{\Delta\phi}=\omega_0\sigma_{\Delta T}$。
