...
> 🖨 **列印／存成 PDF**：桌面瀏覽器按 `Cmd+P`（macOS）或 `Ctrl+P`（Windows/Linux），
> 就能把**目前這一頁**存成 PDF 或直接送印——這招對全站每一頁都有效，不限速查表。
> 導覽列、側邊欄、右側目錄與頁尾會自動隱藏；互動 widget（滑桿、按鈕）的控制項也會隱藏，
> 但 widget 當下畫出的 SVG 圖仍會留在列印結果裡。收合的 `<details>` 區塊（例如
> [exercises](/02_foundations/exercises) 的習題解答）**不會**自動展開，要先在瀏覽器裡
> 點開每一個 ▸ 再列印。
