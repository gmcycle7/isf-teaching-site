...
description: "把全站 101 頁壓成一張 57 節點的 mermaid 依賴圖：每條邊都從每頁的「先備／接下來」麵包屑列直接抽取；12 步學習路徑主幹以金色標出；並附連結度最高的 8 個樞紐頁面與分組節點圖例。"
---
...
這頁只做一件事：把全站 **101 頁**（zh；每頁在 `i18n/en/` 都有英文鏡像）之間「誰要先讀誰」的
關係，畫成**一張圖**。圖不是我手畫的——邊是**逐頁掃描每一頁開頭的「> 先備：…｜接下來：…」
（或「> **麵包屑**：…上游／下游：…」等變體寫法，見下方「怎麼讀這張圖」）麵包屑列**、用程式抽取
markdown 連結後自動生成的，所以它反映的是**網站實際的連結結構**，不是憑印象畫的示意圖。v11
（2026-09-14）把 wave 2/3 新增的 9 頁（`subharmonic_injection`、`injection_locked_division`、
`adpll_tdc_dco`、`cdr_bang_bang_jtol`、`design_recipe`、`lab_40_subharmonic_injection`、
`paper_004_large_injection_transient`、`derivation_autocorrelation_wiener_khinchin`、
`p2_appendix_a_reconciliation`）全部納入——v10 版本卡在 92 頁／56 節點，這批注入群與附錄頁完全
不在圖上，是本頁這次重跑的直接原因。
...
- **方形節點**＝單一頁面（02 基礎、03 核心理論兩章因為是主幹，**逐頁**畫成獨立節點；00/01/05
  ...
  06 設計直覺與 99 附錄收攏度數較低的頁面），因為 101 頁若逐一畫節點會超過可讀上限——分組後全圖
  只剩 **57 個節點**（見下方「節點數如何從 101 頁壓到 57 個」表）。每個分組節點底下實際包含哪些頁，
  列在下方「分組節點圖例」。
...
- **邊的來源與例外（v11 更新）**：全站 **101 頁**中有 **77 頁**在頁首帶標準「先備／接下來」
  （或 lab 頁「麵包屑…上游／下游」）格式的麵包屑列，圖上的每一條邊都逐字從這 77 頁的麵包屑連結
  抽取，不做人工增補（`tank_Q_and_energy_restoration`、`capstone_lc_end_to_end` 這兩頁在更早版本
  沒有標準麵包屑、改用「延伸閱讀」湊邊；v11 重新掃描發現兩頁現在都已經有標準麵包屑列，例外已不
  存在，故不再特別處理）。另外 **24 頁**（00 導覽章多數查閱型頁、01 三個索引頁、04 章
  `lab_01`–`lab_08`＋`numerical_feeling`／`worked_examples`／`interactive_calculator`、99 附錄的
  4 個查閱頁）依設計是「查閱型」頁，頁首不放先備引導，因此不貢獻出邊——但仍以節點身分畫在圖上，
  且可能因為被別頁的麵包屑列為先備／接下來而擁有入邊（例如 `01_paper_map/claims_cross_reference`
  只有「接下來」沒有「先備」）。頁面層級（分組前）萃取到 **270** 條有向邊；收攏成 57 個節點、
  去掉組內部邊與重複後，圖上實際畫出 **204** 條邊。
...
graph LR
  ...
  subgraph SG_04_simulation_labs["04 · 模擬實驗 Simulation Labs"]
    ...
    g_lab_sys(["系統與進階 Labs（lab13–17 · 32 · 34 · 36 · 40 · final_exam）"])
  end
  subgraph SG_05_paper_deep_dives["05 · 論文逐篇精讀 Paper Deep Dives"]
    ...
    n_05_paper_deep_dives_paper_004_large_injection_transient["[P4] 大注入 LC 暫態（剩料）"]
  ...
  subgraph SG_06_design_insights["06 · 設計直覺 Design Insights"]
    ...
    n_06_design_insights_clock_chain_budget["時脈鏈雜訊記帳 ×N/÷N/PLL"]
...
| 04 · 模擬實驗 Simulation Labs | 25 | 3 | 按側邊欄三個子分類收攏（基礎手感／雜訊與抖動／系統與進階；v11 新增 `lab_40_subharmonic_injection` 併入系統與進階） |
| 05 · 論文逐篇精讀 Paper Deep Dives | 7 | 7 | 逐頁（v11 新增 `paper_004_large_injection_transient`；每頁對應一篇論文或其延伸精讀，數量小、意義大，不收攏） |
| 06 · 設計直覺 Design Insights | 25 | 10 | 度數最高的 6 頁（`exercises`、`serdes_clocking_connection`、`pll_noise_budget`、`clock_chain_budget`、`symmetry`、`tank_swing`）逐頁保留，其餘 19 頁按主題收攏成 4 叢（v11 新增 5 頁：`subharmonic_injection`／`injection_locked_division` 併入「真實拓樸與注入」、`adpll_tdc_dco` 併入「系統時脈鏈與參考源」、`cdr_bang_bang_jtol`／`design_recipe` 併入「量測、預算收官與地雷」；同時 `clock_chain_budget` 度數升到 11 反超舊版標準節點 `lc_vs_ring`（現為 10），兩者互換角色——`lc_vs_ring` 這次改收進「真實拓樸與注入」分組） |
| 99 · 附錄 Appendix | 9 | 2 | 收攏成「嚴格數學基礎」與「查閱工具」兩叢（v11 新增 2 頁：`derivation_autocorrelation_wiener_khinchin`、`p2_appendix_a_reconciliation` 併入「嚴格數學基礎」） |
| **總計** | **101** | **57** | |
...
|---|---|---|---|---|---|
| 1 | [白噪如何變成 1/f² phase noise](/03_isf_core_theory/white_noise_to_phase_noise) | 03_isf_core_theory | 23 | 2 | 21 |
| 2 | [設計章習題（含完整解答）](/06_design_insights/exercises) | 06_design_insights | 16 | 12 | 4 |
| 3 | [從 ISF 到 SerDes clocking](/06_design_insights/serdes_clocking_connection) | 06_design_insights | 15 | 11 | 4 |
| 4 | [PLL 完整相位雜訊預算與最佳 loop BW](/06_design_insights/pll_noise_budget) | 06_design_insights | 13 | 7 | 6 |
| 5 | [Phase Noise → Jitter](/02_foundations/psd_phase_noise_jitter) | 02_foundations | 12 | 3 | 9 |
| 6 | [ISF 的 Fourier series（傅立葉級數）](/03_isf_core_theory/fourier_series_of_isf) | 03_isf_core_theory | 12 | 1 | 11 |
| 7 | [時脈鏈雜訊記帳：×N、÷N、PLL、buffer 一頁查表](/06_design_insights/clock_chain_budget) | 06_design_insights | 11 | 7 | 4 |
| 8 | [波形對稱性與 flicker upconversion](/06_design_insights/symmetry) | 06_design_insights | 11 | 6 | 5 |

> 註（v11 重算）：這欄度數改用**單一標準格式**擷取——全站 77 頁的頁首麵包屑列現在都符合「先備／
> 接下來」或 lab 頁「麵包屑…上游／下游」兩種格式之一，不再需要像更早版本那樣為
> `tank_Q_and_energy_restoration`、`capstone_lc_end_to_end` 兩頁另外用「延伸閱讀」湊邊（見上「邊的
> 來源與例外」）；也因此 `capstone_lc_end_to_end` 這次度數大幅降到 3（先備數 2、出邊 1）——它先前
> 排第 2 名是因為舊流程把散落全站、提到它的「延伸閱讀」連結全數算成入邊，並非它自己麵包屑列的
> 真實連結量。
> 第 5–6 名兩頁度數同為 12，用「先比入邊」規則排序：`psd_phase_noise_jitter`（in=3）＞
> `fourier_series_of_isf`（in=1）。度數 11 其實還有兩頁：`06_design_insights/tank_swing`（in=5）與
> `02_foundations/oscillator_phase`（in=3）——同分但入邊數較低，因此緊追在第 8 名之後，未進榜但
> 同樣是名符其實的樞紐。度數 10 有三頁：`03_isf_core_theory/exercises`、
> `05_paper_deep_dives/paper_004_injection_locking_part2`、`06_design_insights/lc_vs_ring`。
> `exercises`（02/03/06 三章）、`serdes_clocking_connection`、`pll_noise_budget`、
> `clock_chain_budget`、`symmetry`、`tank_swing` 因為度數高，在 06 章節裡以**獨立節點**（非分組）
> 方式畫在圖上；`lc_vs_ring` 這次度數降到 10（被新晉標準節點 `clock_chain_budget` 的 11 反超），
> 因此改收進 `g_dsg_topo`（真實拓樸與注入）分組節點，圖例見下方。
...
　[ADC aperture jitter：時脈 jitter 如何吃掉 SNR 與 ENOB](/06_design_insights/adc_aperture_jitter)、[常見錯誤陳列室：12 個真實地雷](/06_design_insights/common_mistakes)、[DJ 與 dual-Dirac 模型](/06_design_insights/dj_dual_dirac)、[FOM 的理論天花板](/06_design_insights/fom_limit)、[相位雜訊量測與 spur](/06_design_insights/measurement_and_spurs)、[Bang-bang CDR：把 ISF 的 σ_t 變成 K_bb、JTOL 與 SSC 追蹤](/06_design_insights/cdr_bang_bang_jtol)、[規格驅動設計配方：從 spec 到 jitter 的 7 步](/06_design_insights/design_recipe)

**系統時脈鏈與參考源 System Clocking & References**（節點 `g_dsg_sys`）：
　[參考源振盪器：crystal 與 MEMS 的 phase noise](/06_design_insights/reference_oscillators)、[Sampling / sub-sampling PLL：把 divider 踢出迴路](/06_design_insights/sampling_pll)、[全數位 PLL：TDC 與 DCO 的量化雜訊也是同一條 ISF 記帳](/06_design_insights/adpll_tdc_dco)

**真實拓樸與注入 Real Topologies & Injection**（節點 `g_dsg_topo`）：
　[注入鎖定的雜訊整形與 injection pulling 頻譜](/06_design_insights/injection_locking_noise)、[Quadrature 產生與 coupled-oscillator phase noise](/06_design_insights/quadrature_and_coupled_oscillators)、[真實拓樸的 ISF：cross-coupled LC VCO、Colpitts、CMOS ring stage](/06_design_insights/real_oscillator_topologies)、[Tuning line 與 supply pushing 的相位雜訊](/06_design_insights/varactor_tuning_supply_pushing)、[Subharmonic injection：從 impulse-train 到 ×N 倍頻（ILCM／ILFM）](/06_design_insights/subharmonic_injection)、[注入鎖定除頻（ILFD）：M:N 次諧波鎖定與設計](/06_design_insights/injection_locked_division)、[從 ISF 看 LC vs ring oscillator](/06_design_insights/lc_vs_ring)
...
　[Floquet / adjoint / PPV：ISF 的嚴格基礎](/99_appendix/derivation_floquet_ppv)、[Leeson 模型推導與 ISF 對照](/99_appendix/derivation_leeson)、[嚴格 LTV 框架：Zadeh 時變傳函與 harmonic transfer matrix](/99_appendix/ltv_htm)、[數學工具箱 Math Identities](/99_appendix/math_identities)、[自相關 → Wiener-Khinchin 嚴格推導](/99_appendix/derivation_autocorrelation_wiener_khinchin)、[[P2] Appendix A 逐字對帳：時域自相關路與 Khinchin 路](/99_appendix/p2_appendix_a_reconciliation)

## 重點回顧

- 這張圖的邊 100% 來自每頁自己聲明的「先備／接下來」麵包屑列（v11 之前兩頁的「延伸閱讀」例外
  已不存在，見上）——它是網站**實際**依賴結構的鏡子，不是重新設計的理想教學順序（理想順序仍以
  [learning_path](/00_overview/learning_path) 十二步為準，圖上以金色標出）。
- 101 頁 → 57 節點：02／03 兩個核心章節逐頁保留（教學規格要求），04 全收攏、06／99 依連結度
  部分收攏，讓圖維持在「一眼掃得完」的規模。v11 新增的 9 頁（注入群 5 頁、lab_40、
  paper_004_large_injection_transient、附錄 2 頁）全部依同一套規則收攏或逐頁畫入。
- 連結度最高的樞紐不在 00/01（查閱型字典頁本身很少被其他頁列為「先備」，麵包屑度數自然低），
  而是白噪→1/f²、章末習題（06）、SerDes clocking、PLL 雜訊預算、Phase Noise→Jitter、ISF 傅立葉
  級數、時脈鏈記帳、波形對稱性這幾頁——它們是全站教學邏輯真正的收束點。
