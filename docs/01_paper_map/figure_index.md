...
| `isf_shaping_division.png` | [injection_locked_division](/06_design_insights/injection_locked_division)、[paper_004_injection_locking_part2](/05_paper_deep_dives/paper_004_injection_locking_part2)（文字連結） |
...
## v8/v10 補完（6 張先前漏登的生成圖）

以下 6 張圖在 wave 2/3（R11-17 QVCO、R11-19 lab_43、R11-26 jitter 核、以及 fig_p1_fig2930_replica／
fig_subharmonic_injection 兩支 v8 腳本）陸續生成、嵌入頁面，但先前未列入本頁主表與 used_in 交叉表，
本次盤點（R11-42）一併補登。

| Figure | Generated By (script / function) | Formula Behind It | Source | Teaching Message | Toy? |
|---|---|---|---|---|---|
| `p1_fig2930_replica.png` | `simulations/fig_p1_fig2930_replica.py` / `main`（`draw_fig29`+`draw_fig30`；左 μ=2.0、右 μ=0.2 van der Pol） | 左：實際積分的 kick-and-relax 軌跡、以晚期零交越量 $\Delta\phi$；右：重用 Method A（impulse）/B（[P1] Eq.(37) $\Gamma=f'/(f'^2+f''^2)$）/C（Eq.(38) $\Gamma\propto f'$）三法 | [P1] Fig. 29, p.192（逐字標題 *"State-space trajectory of an nth-order oscillator."*）；Fig. 30, p.193（*"ISF's obtained from different methods."*） | **概念復刻，非原圖數位化**：原圖 29 為示意手繪、原圖 30 振盪器未載明；本站以 van der Pol 實算並誠實揭露與原圖的差異（原圖 1st≈2nd、本站 B≈C） | toy（van der Pol，同 lab_15/25/isf_three_methods 機器） |
| `subharmonic_injection_ilcm.png` | `simulations/fig_subharmonic_injection.py` / `main`（savefig 於行 447；4 panels） | 矩形脈衝諧波 sinc 包絡；per-pulse map $\omega_L=q_{inj}\vert\tilde\Gamma\vert_{max}/(NT_0)$；realignment factor $\beta=-q_{inj}\tilde\Gamma'(\theta_{ss})$；一階離散迴路 $H_{osc}(z)=(1-z^{-1})/(1-(1-\beta)z^{-1})$ | [P3] Sec. IV footnote 7, p.2112；[P4] Eq.(28)–(30), p.2129 | lock range $\propto1/N$；輸出雜訊被一階高通整形、corner $\approx\beta f_{ref}/2\pi$；reference spur 與 $\beta$ 無關 | toy（phase-only、弱注入 pedagogical toy；與 `subharmonic_injection.png`（lab_40）為同一頁互補的兩張圖） |
| `classc_classf_isf.png` | `simulations/lab_43_classc_classf_isf.py` / `main` | class-C：$\Gamma_{eff,rms}^2=(1/\pi)(\phi-\sin\phi\cos\phi)$（[P1] Eq.(27) $\alpha$ 開關窗）；class-F：$\Gamma(\theta)=f'/(f'^2+f''^2)$（[P1] Eq.(37)）對 $f=\sin\theta+\zeta\sin3\theta$，與 Babaie–Staszewski 閉式 $\Gamma_{rms}^2=\frac12\frac{1+9\zeta^2}{(1+3\zeta)^2}$ 核對 | [P1] Eq.(27),(37),(38)；Babaie-Staszewski, JSSC 48(12) 2013, Eq.(3), p.3121（**外部文獻，非本站 5 篇 PDF**） | class-C 導通角變窄／class-F 三次諧波 $\zeta=1/3$ 都能把 $\Gamma_{rms}^2$ 從 1/2 壓到 1/4（−3 dB）；換到最差（ZC 對齊）窗則完全不省 | toy（拓樸層級：理想 LC tank 的 $-\sin$ 精確、class-F ISF 為 [P1] 波形閉式近似） |
| `coupled_qvco.png` | `simulations/lab_42_coupled_qvco.py` / `main`（3 panels） | 互注入 [P3] Adler 方程對：$\Omega(\theta)=\frac12I_{inj}\vert G_1\vert\cos(\theta+\angle G_1)$（Eq.(34)）、$\omega_L=\frac12I_{inj}\vert G_1\vert$（Eq.(35)）；穩態 $\cos\psi^*=\Delta\omega_0/(2\omega_L\sin\phi_c)$ | [P3] Eqs.(30),(33)–(35), p.2113–2114 | coupled QVCO＝兩條互注入的 Adler 方程；$\phi_c=90°$ 時精確 arcsin 解 vs 線性近似 $(Q/m)(\Delta\omega_0/\omega_0)$；陽春 parallel QVCO（$\phi_c=0$）在 phase-only 模型下不鎖 | toy（phase-only、理想 LC ISF $-\sin$） |
| `isf_shaping_division.png` | `simulations/fig_isf_shaping_division.py` / `main`（2×3 panels） | 雙高斯脈衝 ISF 造型；$\omega_L^+=\frac12I_{inj}\vert\tilde\Gamma_2\vert$（[P4] Eq.(30) 的 $N=2$ 特例）；ISF 前五個傅立葉係數 $\vert c_n\vert/\Gamma_{rms}$ | [P4] Sec. VII-A, Fig. 15–16, Table IV, p.2132–2134 | 波形不對稱同時養出 $c_0,c_2,c_4$；半波對稱（[P4] footnote 14）使二次諧波精確互消 → 鎖不住 ÷2 | toy（單端 inverter-chain ring 以雙高斯脈衝簡化，非 transistor-level） |
| `jitter_kernel_shapes.png` | `simulations/lab_24_jitter_kernels.py` / `make_figure_kernels` | 三種 jitter 核 $\vert H(f)\vert^2$：TIE $=1$、N-period（$N=1$）$=4\sin^2(\pi fT)$、cycle-to-cycle $=16\sin^4(\pi fT)$，低頻漸近線 $\propto f^2,f^4$ | [P2] Eq.(8), p.792（jitter 定義）；本站推導（jitter_kernels.md 第 3 步） | 三種 jitter 定義＝對 $S_\phi(f)$ 的三種頻域濾波核；cycle-to-cycle 對低頻雜訊最不敏感（$f^4$ 抑制） | model（標準 DSP kernel，convention-focused，同 `jitter_kernels_mc.png`/`jitter_two_regime.png` 腳本） |

## 重點回顧

- 60 張生成圖**全部可追溯**到 script／function／參數／公式／來源，且**沒有孤兒圖**
  （每張至少被 1 個頁面嵌入；`comm -23` 核對 `static/figures/*.png` 與本頁登錄名單、以及登錄名單與
  `docs/` 內實際 `![...](/figures/...)` 引用，兩者皆為空集合）；除 `phase_noise_to_jitter_integration.png` 外，
...
| `jitter_kernels_mc.png` | `lab_24_jitter_kernels.py` / `make_figure` | 三種 jitter 核：TIE（kernel=1）、N-period（$4\sin^2(\pi fNT)$）、cycle-to-cycle（$16\sin^4(\pi fT)$），單一 $S_\phi$ one-sided 慣例下 Monte-Carlo 對照閉式解 | [P2] Eq.(8), p.792；Eq.(11)/(12), p.793（相位版 $\kappa$＝Eq.(11) 開根號；印刷 Eq.(12) 為 $\kappa_t=\kappa/\omega_0$，分母含 $\omega_0$） | 三種 jitter 定義對應三種頻域核；白噪 FM 下 $\sigma_{\Delta\phi}^2(N)=\kappa^2 NT$ 精確成立 | model（標準 DSP kernel 積分 + Monte-Carlo，convention-focused） |
