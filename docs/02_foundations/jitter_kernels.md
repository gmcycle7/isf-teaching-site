...
description: 在「單邊 S_φ、∫₀^∞」單一慣例下，從 φ(t+NT)−φ(t) 一步步推出 TIE 核 1、period 核 4sin²(πfNT)、cycle-to-cycle 核 16sin⁴(πfT)；白噪 FM 封閉式精確回收 [P2] Eq.(8)/(11) 的 σ_Δφ=κ√(NT)；flicker 1/f³ 給含 log 項的封閉式；再合成 [P2] Fig.16 的兩段式 σ(Δt)=√(κ²Δt+ζ²Δt²)、corner Δt_c=κ²/ζ² 與頻域 1/f³ corner 的映射；lab_24 Monte-Carlo 驗證比值 ≈1.00，正式關閉 worked_examples 例 C3 的前置常數 TODO。[P2] Appendix A（Eq.(40)–(51)，p.802–803）的逐字轉錄與逐因子對帳已獨立成頁（p2_appendix_a_reconciliation），本頁只留指標；第 3 步後新增三核形狀圖（TIE=1、4sin²、16sin⁴，標峰值與 f²/f⁴ 漸近線）。
---
...
下表三行分別對應第 2 步定義的三種 jitter：**TIE**＝對理想時鐘的絕對邊沿誤差（0 階，不差分）、
**N-period**＝相位的一階差分（間隔 $NT$）、**cycle-to-cycle**＝相位的二階差分。
...
把三個 boxed 核的形狀畫在一起，直接看峰值與低頻滾降：

![TIE、N-period、cycle-to-cycle 三個 jitter 核的正規化 |H(f)|²：TIE 平坦於 1（不濾）、period 核峰值 4、f→0 時 ∝f²、cycle-to-cycle 核峰值 16、f→0 時 ∝f⁴](/figures/jitter_kernel_shapes.png)

- **公式**：核 (a)＝$1$、核 (b)＝$4\sin^2(\pi fT)$（$N=1$）、核 (c)＝$16\sin^4(\pi fT)$
  ——即上面三個 boxed 公式的被積函數，橫軸為正規化頻率 $fT$（無因次）。
- **script / function**：`simulations/lab_24_jitter_kernels.py`（`make_figure_kernels`），
  輸出 `static/figures/jitter_kernel_shapes.png`。
- **怎麼讀**：log-log 圖上三條曲線的峰值分別是 $1$（TIE 不濾，恆定）、$4$（period，
  在 $fT=0.5$ 達峰）、$16$（c2c，在 $fT=0.5$ 達峰，圖上以水平虛線與數字標出）；
  低頻虛線是 $f\to0$ 的漸近線 $(2\pi fT)^2$（period）與 $(2\pi fT)^4$（c2c）——
  分別對應「一階高通」與「二階高通」，差分階數越高、越不吃 close-in phase noise。
  這張圖濃縮了本節三個 boxed 公式的全部形狀資訊，供之後任何一步引用核形狀時對照。
...
- **單位（重要、常被搞混）**：這裡的 $\kappa$ 是**相位版**，
  ...
  裡面**沒有 $\omega_0$**（[P2] p.793 印刷的 Eq.(12) 則分母**含** $\omega_0$，就是下面的時間版；v11 重核）。**時間版**由 [P2] Eq.(10), p.793 的
...
## 論文原生推導：[P2] Appendix A（逐字對帳，已獨立成頁）

逐字對帳 [P2] Eq.(40)–(51) 見 [p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation)——
[P2] **Appendix A "Relationship Between Jitter and Phase Noise"**（p.802–803）的逐字轉錄＋逐因子對帳：
白噪時域自相關路 Eq.(40)–(44)（＝上面 4.2 節，收斂到主文 Eq.(11)）與自相關＋Khinchin 路
Eq.(45)–(51)（＝上面路 A，另加兩條白噪特例推論），附逐步對照表、誰跳過了什麼的誠實記帳、
以及 Python 把兩條路各算一遍、同一顆振盪器得到同一個 $\kappa^2\Delta T$ 的數值重演。
第 3、4 步是本站自己的推導；那一頁證明論文的兩條路與這裡完全互鎖。
...
[P2] 把這件事直接「量」給你看——這就是著名的 Fig. 16 兩段式 log-log 圖。**本站慣例**：本節
（及全頁）的 $\zeta$ 是 [P2] Fig.16 的符號，非 PLL 阻尼比 $\zeta$（見 pll_noise_budget 等頁）。
...
- jitter ← phase spectrum（自相關＋Khinchin 路線）：[P2] Eq.(46)–(49), p.803
  ...
  對帳見 [p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation)）；白噪特例
  ...
  Eq.(51) $\sigma_{CTC}=\tfrac{f}{f_0^{1.5}}\cdot10^{-\mathcal{L}\{\Delta f\}/20}$（照排——分子印作 $f$；由 Eq.(50)×$\sqrt T$ 的因次可知應讀 offset 頻率 $\Delta f$，$\Delta$ 為印刷遺漏，見 [p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation) A.2）——**印刷式無 $\sqrt2$**：其 $\sigma_{CTC}=\kappa\sqrt{T}$ 是「一個週期的累積」（即本頁的 $\sigma_P$）；若取「相鄰週期差」定義（本頁 $16\sin^4$ 核）則再乘 $\sqrt2$。兩種定義並存於文獻，本頁已分開命名。）
