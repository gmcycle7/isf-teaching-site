...
下面三題把整頁的鏈條與權重核各跑一遍，第三題再示範獨立雜訊源的功率疊加。格式：題目 → 逐步代入
（帶單位）→ 結果 → dimension check → 一行 Python 驗證（引用 `simulations/common/`）。
...
### 例 E（兩個雜訊源相加）：獨立源在 $S_\phi$（功率）上疊加

> **題目**：真實振盪器從來不只一個 noise 源。這題示範**多源 superposition**——兩個獨立白噪源
> 同時打進同一個理想 LC 節點，該怎麼合成總 phase noise？沿用
> [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) 例 1 的振盪器參數
> （$f_0=5$ GHz、$\Delta f=1$ MHz、$q_{max}=1$ pC），節點上疊加兩個互相獨立
> （statistically independent）的白噪電流源：
...
是多少？套用的公式仍是本頁與 [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise)
共用的 [P1] Eq.(21)。
...
**第 1 步：Device A 單獨算（就是 [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) 例 1 的算法，套 [P1] Eq.(21)）。**
...
$\mathcal{L}_A=10\log_{10}(1.583\times10^{-15})\approx-148.0\ \text{dBc/Hz}$（與
[white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) 例 1 一致——同一組參數）。
...
**Dimension check：** $\ell_A$、$\ell_B$ 分別跟
[white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) 例 1／例 2 一樣是
$\Gamma_{rms}^2/q_{max}^2\cdot S_i/(4\Delta\omega^2)$ 的形式，因次都化簡為 $\text{s}$（per-Hz，見該頁例 1
的 dimension check），兩個同因次的量才能相加；相加後仍是 $\text{s}$，取 $10\log_{10}$ 讀作 dBc/Hz ✓。
...
$\mathcal{L}_{total}$ 三者會一起各加 $3$ dB，彼此的 **3.0 dB 差距不變**——這再次呼應
[white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) 的 factor-of-2 註記：常數慣例
不影響 scaling／相對關係。多源 superposition 的完整規則與 cyclostationary $\Gamma_{eff}$ 的推導見
[effective_isf](/03_isf_core_theory/effective_isf)；完整函式庫：`simulations/common/noise_utils.py`、
...
- **例 E**：獨立雜訊源要在 $S_\phi$（線性功率）相加，不能在 dBc/Hz 相加；兩個功率相等的源合起來只多
  $+3.0$ dB，不是把兩個 $\Gamma_{rms}$ 大小直接比較就能猜對。
