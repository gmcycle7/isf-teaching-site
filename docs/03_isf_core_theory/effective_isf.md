...
（claim C13）。為了讓你知道整張地圖長怎樣，這裡給直覺；正式 citation 已補（見下方誠實聲明，[E2]/[E3]）：
...
> **範圍說明**：PPV 的嚴格定義與 $\dot\phi=v_1^T B\xi$ 形式即 **[E2] Demir 2000** 的核心結果（全篇即此主題，TCAS-I 47(5):655–674，DOI 10.1109/81.847872）；$v_1(t)$ 與 $\Gamma/q_{max}$ 的對應（「ISF = PPV 在電荷注入方向的標量化」）見 [E2] 與 Kärtner [E3]。本站只引述、不重證（屬外部文獻範圍）。
