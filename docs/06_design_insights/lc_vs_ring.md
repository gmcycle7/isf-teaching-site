...
- **ring**：$N$ 級 inverter delay + 每級獨立 timing noise；ISF 用 triangular（三角形）toy 形狀，
  峰值 $\sim1/\sqrt{N}$、能量集中在 transition（[P2] Fig. 5）。累積 jitter 用 random walk 模型
...
- **單位檢查**：$[\sqrt{\text{s}}]\cdot[\sqrt{\text{s}}]=[\text{s}]$ ✓（$\kappa$ 單位 $\sqrt{\text{s}}$）。
- $\kappa^2\propto\Gamma_{rms}^2/q_{max}^2\cdot\overline{i_n^2}/\Delta f$（[P2] Eq.(12), p.793，已核實：$\kappa=(\Gamma_{rms}/q_{max})\sqrt{(\overline{i_n^2}/\Delta f)/2}$）——
  同一個核心比值又出現。
...
- **比較**：理想單源下 LC $-148$ vs ring $-89$ → LC 約**好 59 dB**。（兩數來自兩篇論文各自的自然參數化：
