...
上面的 $1/\sqrt{M}$ 收斂只是文字描述，這裡用一個**可重現的數值模擬**把它做出來，親眼看「單通道量到儀器本底、互相關把 DUT 真實本底挖出來」是怎麼發生的（完整程式碼：`simulations/lab_35_xcorr_measurement.py`；圖：`static/figures/xcorr_floor.png`）。

**方法論走一遍**：

1. **合成一個「真實」DUT 頻譜**：用本站 canonical 數值（例 B：$q_{max}=1$ pC、$\Gamma_{rms}=0.5$，[P1] Eq.(21) 給出 $\mathcal{L}(1\text{ MHz})=-148.0$ dBc/Hz，即 $S_\phi(1\text{ MHz})=2\times10^{-14.8}=3.17\times10^{-15}$ rad²/Hz）當作 $1/f^2$ 中段，再疊上一個比它低 $20$ dB 的白色本底（$S_{DUT,floor}=3.17\times10^{-17}$ rad²/Hz）——一個誠實的「兩段式」toy DUT 頻譜：近載波 $1/f^2$、遠端轉平。這是**已知答案**，才能檢驗量測法有沒有把它還原出來。
2. **兩條獨立通道各自加儀器白噪**：$y_1(t)=\phi_{DUT}(t)+n_1(t)$、$y_2(t)=\phi_{DUT}(t)+n_2(t)$，$n_1\perp n_2\perp\phi_{DUT}$，每路本底設在比 DUT 本底高 $15$ dB（$S_{instr}=10^{1.5}\times S_{DUT,floor}\approx1.00\times10^{-15}$ rad²/Hz）——對應方法比較表裡「單通道本底 $=$ DUT 本底 $+$ 儀器本底」的情境。
3. **兩種頻譜同時算**：單通道 auto-spectrum $S_{yy}=\langle|Y_1|^2\rangle$（$M$ 段平均），以及互功率譜 $S_{y_1y_2}=\langle Y_1Y_2^*\rangle$（同樣 $M$ 段平均），$M$ 取 $1,4,16,64,256,1024$（4 的冪次）。
4. **在一個遠離 $f_{ref}$ 的高頻頻帶**（$80$–$95$ MHz，這裡 DUT 的 $1/f^2$ 裙邊已經衰減到 DUT 本底的 $1.5\%$ 以下，可視為「只剩本底」）量互相關殘餘本底隨 $M$ 的變化，並對 $M\le64$（此範圍內殘餘本底仍明顯高於 DUT 真實本底、可視為「不相關雜訊主導」）做直線回歸，比對規範式 $\Delta_{floor}=5\log_{10}M$ 的斜率。
5. **在 $M=1024$ 比較還原頻譜與真實 DUT 頻譜**，看互相關能不能把單通道量不到的 DUT 真實本底「挖」出來。

**關鍵結果（`# ->` 標記處為程式輸出）**：

| $M$ | 互相關殘餘本底 [rad²/Hz] | 相對 $M{=}1$ [dB] | 理論 $-5\log_{10}M$ [dB] |
|---|---|---|---|
...
對 $M\le64$（不相關殘餘仍明顯高於 DUT 本底的範圍）做斜率擬合：**擬合斜率 $=-4.73$ dB/decade**，對比理論 $-5.00$ dB/decade，**吻合度 $=0.946$**（# -> `slope match: fitted/theory = 0.9463`）——在有限樣本（$\sim300$ 個獨立頻率 bin、8 次獨立實現平均）下，$1/\sqrt{M}$ 定律被乾淨驗證。

到 $M=256$、$1024$ 兩點，數字開始偏離理論直線（$-11.15$、$-13.05$ dB，比理論的 $-12.04$、$-15.05$ dB「淺」）——**這不是誤差，是物理**：因為模擬只讓 $M$ 掃到 $1024$（$\sqrt{M}$ 只有 $32$ 倍、約 $15$ dB 的改善空間），而單通道本底 margin 也設在 $15$ dB，兩者數量級接近，所以殘餘不相關本底 $S_{instr}/\sqrt{M}$ 到 $M=1024$ 時已經**逼近**（而非遠低於）DUT 真實本底 $S_{DUT,floor}$，殘餘曲線因此彎向真實本底、不再繼續遵守純 $1/\sqrt{M}$ 直線——這正是互相關法「成功把本底壓到 DUT 真實值附近」的訊號，而非失敗。

**還原效果**（$M=1024$，見下圖 (b)）：

- 在 DUT 本底區（$90$ MHz 附近）：真實 $\mathcal{L}=-167.95$ dBc/Hz；**單通道**量到 $-152.74$ dBc/Hz（被儀器本底蓋住，差了 $15.2$ dB，量到的其實是儀器）；**互相關**（$M=1024$）量到 $-166.15$ dBc/Hz，**只差 $+1.79$ dB**（# -> `residual error at DUT floor after M=1024 averaging: +1.79 dB`）——把被儀器本底完全蓋住的 DUT 真實本底，用兩條獨立通道相關 + 平均，幾乎完整地「挖」了回來。
- 在 $1/f^2$ 中段（$1$ MHz）：DUT 本身訊號夠大，單通道與互相關結果都接近真值（$-147.80$ dBc/Hz 真值 vs. 單通道 $-143.87$、互相關 $-144.43$ dBc/Hz）——這裡本來就不需要互相關，示範的重點在**本底**（遠端），不在訊號強的地方。

![Cross-correlation 量測模擬：本底 vs 平均次數 M 的 1/√M 收斂，以及 M=1024 還原頻譜 vs 真實 DUT](/figures/xcorr_floor.png)

**誠實聲明**：這裡驗證的只是互相關背後的**統計/DSP 機制**（$1/\sqrt{M}$ 收斂本身是通用複數高斯統計的性質，不是本站 5 篇 PDF 的內容，也不是 ISF 物理），且模型中兩通道的儀器本底被設定為**完全不相關**。真實儀器上，$M\to\infty$ 並不能把本底無限壓低——**通道間殘餘相關**（共用參考時脈分配、共用供電/接地、熱耦合等）會設下一個模擬中未建模的物理下限（**外部工程文獻，非本站 5 篇 PDF**；上面模擬只示範理想 $1/\sqrt{M}$ 機制與一個有限 $M$ 下的真實結果，不代表可無限逼近）。
