...
> **記帳口訣**：**相關 ↔ 振幅相加（功率 $\times N^2$ 除以 …）→ 不享受平均；不相關 ↔ 功率相加 →
> 享受 $-10\log_{10}N$。** 這跟第 1 節 $\div N$ 改善 $20\log_{10}N$（相位被除以 $N$，是
...
| 兩顆 VCO 近乎相同、失配小 | I/Q error $\propto 1/(mQ)$、$\sim 3$ dB 上限可逼近 | 大失配 → I/Q error 大、甚至失鎖（pulling） |
