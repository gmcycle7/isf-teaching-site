...
- **P0 corrections (reader-visible errors)**: CDR jitter **tolerance** was labelled $\lvert H_{lp}\rvert^2$ (that is the transfer; tolerance follows $1-H_{lp}=H_{hp}$; new JTOL subsection with worked numbers); isf_definition stated the opposite sign of [P3] Eq.(30) from paper_003 (the paper prints **+**; [P4] Eq.(26) $\tilde\Gamma_1\angle90°$ has the same sign as this site's $-\sin\theta$); effective_isf gave 0.22 and 0.31 for the same $\Gamma_{rms}^{eff}$ scenario (unified to 0.31 / $-152.2$ dBc/Hz); final-exam Q7 accepted the given $-155$ and showed "±0%" (now asks 1 MHz: $-151.47$ dBc/Hz; NumericQuiz shows ±0.1%); **[P4] APF symbols per the rendered pages**: $\tilde\Lambda(\varphi)$ = amplitude ISF (1/C, Eq.(18)/(24)), $\Delta(\varphi)=\tau_0\tilde\Lambda$ = APF (1/A, Eq.(19)/(25)), $\Delta_1=(\tau_0/q_{max})\angle0°$ (Eq.(26)), unified across 16 files (the plan's Λ came from blank Greek glyphs in the raw text); home page "9 steps" → "12 steps"; **[P1] equation page numbers**: Eq.(1)–(5) p.180, Eq.(6)–(9) p.181, Eq.(10)–(11) p.182, Eq.(12)–(18) p.183 (44 edits in 27 files, checked page by page); lab_24 notebook regenerated (Part 5); the two [P2] Eq.(12)/Eq.(6) attribution fixes above.
- **Paper sections that were untaught (fable)**: [P1] Sec.III-F Eq.(28)–(29) collapsing to the Leeson form and the Craninckx–Steyaert 2× gap (derivation_leeson Step 6); [P2] Eq.(27)–(30) short-channel model + Sec.VIII measured validation (Table I–III, #3/#12 recomputed); [P4] Sec.VII ISF shaping for division (half-wave symmetry killing $c_2$, Table IV, new figure isf_shaping_division); [P3] deep dive gains silicon validation (Fig.12–16), the Adler intermediate algebra, Eq.(6)–(10) and footnote 3.
- **New system-level pages**: [adpll_tdc_dco](/06_design_insights/adpll_tdc_dco) (TDC/DCO quantization floors and budget), [cdr_bang_bang_jtol](/06_design_insights/cdr_bang_bang_jtol) ($K_{bb}$, JTOL mask, SSC), [design_recipe](/06_design_insights/design_recipe) (spec → FOM feasibility → LC/ring → Q/C/L/Rp → $q_{max}$ → $\mathcal{L}(\Delta f)$ → jitter, one worked chain); charge-pump type-II design equations (lab_13 §2 + `pll_utils.design_type2`) and the PLL jitter–power FOM; final exam warm-ups 0a/0b + learning-path tags; lab_42 (coupled QVCO, reproduces 1.9°); lab_43 (class-C/D/F ISF waveform engineering).
- **Flow and metadata**: learning path re-attaches 9 orphaned prerequisite pages; glossary gains 19 injection-locking/measurement terms; notation.md gains the injection/PLL/Allan/SerDes symbol families and disambiguates α/β/Δω/ω_c/τ₀/ζ/N/M (one-line site-convention notes at first use); chapter landing pages; python_environment/README aligned with the real files; breadcrumb consistency (9 chapter-03 pages + 4 chapter-end pages); two new appendix pages [p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation) and [derivation_autocorrelation_wiener_khinchin](/99_appendix/derivation_autocorrelation_wiener_khinchin) (slimming jitter_kernels and white_noise_to_phase_noise); reciprocal mixing/EVM subsection; datasheet piecewise phase-noise integration (12 kHz–20 MHz: 8.45 ps) + three cross-correlation pitfalls; DLL rule 5 and PI quantization; PAM4 28/56 GBd and 12-bit 1 GS/s ADC examples; oscillator selection table; 7 leaked-answer quizzes re-asked, 4 widget host pages gain default-anchored quizzes; theory_map rebuilt (v10 injection group + v11 pages); figure_index 60 figures with zero orphans (6 were missing); references.md numbering made consistent; robots.txt.
- **English edition**: 102 pages with zero structural parity mismatches; 76 Translator's notes under figures generated by 47 scripts with Chinese axis labels (new tool `scripts/list_cjk_figures.py`); leftover Chinese inside `\text{}` removed.
- **New tools**: `scripts/validate_mdx.mjs` (Docusaurus-equivalent MDX + GFM + math compile check, `{#id}` aware), `scripts/check_en_parity.py` (12 structural metrics).
- **Process notes**: two interruptions by API usage limits (wave 2 resumed from the interrupted state rather than redone); wave-2 reviews used sonnet to save budget, with every number re-verified by the gate; the R11-05 implementer's deviation to Δ was correct (the site owner rendered the pages); the κ Eq.(12) case is this site's **fourth** "verified-stamp was wrong" incident — a "verified" tag must itself be re-verified, and the raw text's Greek glyphs are blank.
- Scale: **102 pages × 2 locales, 60 figures, 52 simulations, 21 interactive components, 211 verifiable examples (0 wrong)**.

- **[P2] Eq.(12), p.793 as printed contains $\omega_0$**: $\kappa=\dfrac{\Gamma_{rms}}{q_{max}\,\omega_0}\sqrt{\tfrac12\,\overline{i_n^2}/\Delta f}$, units $\sqrt{\text{s}}$ — the **timing** constant of Eq.(8) $\sigma_{\Delta T}=\kappa\sqrt{\Delta T}$ (assembled from Eq.(10) $\sigma_{\Delta\phi}=\omega_0\sigma_{\Delta T}$ and the phase variance of Eq.(11)). This site's phase version $\kappa_\phi=(\Gamma_{rms}/q_{max})\sqrt{S_i/2}$ (rad/√s; canonical $\kappa_\phi^2=0.125\ \text{rad}^2/\text{s}$, $D=\kappa_\phi^2/2$) is correct physics and **not a single number on the site changes**; only the attribution was wrong: v4–v5 had recorded "Eq.(12) has no $\omega_0$" as verified (v4 even blocked the audit's correct objection as a "false fix") and diffusion_dictionary called it the paper's "unit trap" — that was this site's misreading, not the paper's. In v11 the site owner re-read the rendered PDF and corrected paper_002, diffusion_dictionary, beyond_lorentzian, lc_vs_ring, jitter_kernels, equation_index, figure_index, tank_swing, math_identities, references and derivation_floquet_ppv (zh + EN); the relation is stated uniformly as $\kappa_\phi=\omega_0\kappa_t$, $\kappa_t=\kappa_\phi/(2\pi f_0)$ (canonical $1.125\times10^{-11}\ \sqrt{\text{s}}$).
- **[P2] Eq.(6), p.792 is the $/2$ family**: as printed, $\mathcal{L}\{f_{off}\}=\Gamma_{rms}^2/(8\pi^2f_{off}^2)\cdot(\overline{i_n^2}/\Delta f)/q_{max}^2$, whose denominator $8\pi^2f^2=2\Delta\omega^2$ sits **3 dB above** the $/4$ SSB family of [P1] Eq.(21) (this site's $-148$ dBc/Hz vs $-145$). The paper_002 differential-ring numerical example used to say "same family as the 4 of [P1], time-domain /2 is +3 dB on top" — backwards; corrected in v11: Eq.(34) is collected from $2N\times$Eq.(6), so $-82.7$/$-78.0$ dBc/Hz are $/2$-family values (the $/4$ family gives $-85.7$/$-81.0$), and $\Delta\mathcal{L}=+4.77$ dB is unaffected (the same convention flag in lc_vs_ring example 3 was corrected in step).
- **One citation fixed in passing**: math_identities attributed $\sigma_{\Delta t}=\kappa\sqrt{\Delta t}$ to "[P2] Eq.(10), p.793" — on the rendered PDF that is Eq.(8), p.792; Eq.(10), p.793 is the phase-to-time conversion $\sigma_{\Delta\phi}=\omega_0\sigma_{\Delta T}$.
