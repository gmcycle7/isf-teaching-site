...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Title reads: "Allan deviation：每種 FM 雜訊有特徵斜率（虛線 = 理論斜率）" = Allan deviation: each FM noise type has a characteristic slope (dashed = theoretical slope).
...
<NumericQuiz
  prompt="Keeping the explorer at its default settings (white FM h₀=10⁻¹⁹, random-walk FM h₋₂=10⁻²⁴ enabled, seed=1234), what does the gray dashed line (the analytic overlay, independent of the noisy blue measured points) give for σ_y(τ) at τ = 64 s? (Hint: add the white-FM and random-walk-FM closed forms from Step 4.)"
  answer={3.47e-11}
  tol={0.03}
  hint="White FM contributes σ²=h₀/(2τ); random-walk FM contributes σ²=(2π)²/6·h₋₂·τ. The two independent processes' variances simply add, then take the square root. Scientific notation is fine, e.g. 3.47e-11."
  solutionNote="At τ=64 s, white FM contributes σ²≈7.81×10⁻²², random-walk FM contributes σ²≈4.21×10⁻²²; adding and taking the square root gives ≈3.47×10⁻¹¹ — right around the 'bathtub' minimum for this (h₀,h₋₂) pair (white FM dominates the rise at smaller τ, random-walk FM at larger τ)."
/>

<details>
<summary><strong>Verification: reproducing the dashed-line value at τ=64 s</strong> (mirrors AdevLiveExplorer.js's analyticSigma)</summary>
...
# widget defaults: logHWhite=-19, logHRW=-24, rwOn=true, flickerOn=false, seed=1234
# (AdevLiveExplorer.js:196-201)
h_white = 10 ** -19
h_rw = 10 ** -24
h_flicker = 0.0
tau0 = 1  # TAU0, arbitrary "s" unit

tau = 64 * tau0  # one of the octave-spaced m values (m=64)
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis label/title read: "σ_y(τ)（分數頻率，無因次）" = σ_y(τ) (fractional frequency, dimensionless); "flicker-FM floor 絕對高度驗證（canonical 5 GHz、f_c=3.2 kHz）" = flicker-FM floor absolute-height verification (canonical 5 GHz, f_c = 3.2 kHz).
