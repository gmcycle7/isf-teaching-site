...
The three rows below correspond to the three jitters defined in Step 2: **TIE** = the
absolute edge error against an ideal clock (0th order, no differencing), **N-period** =
the first-order difference of the phase (spacing $NT$), **cycle-to-cycle** = the
second-order difference of the phase.
...
Plotting the shapes of the three boxed kernels together makes the peaks and low-frequency roll-off easy to read at a glance:

![Normalized |H(f)|² of the three jitter kernels TIE, N-period, and cycle-to-cycle: TIE is flat at 1 (no filtering), the period kernel peaks at 4 and goes as ∝f² as f→0, the cycle-to-cycle kernel peaks at 16 and goes as ∝f⁴ as f→0](/figures/jitter_kernel_shapes.png)


> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/title read: "fT（正規化頻率，無因次；N-period 核以 N=1 繪示，一般為 fNT）" = fT (normalized frequency, dimensionless; the N-period kernel is drawn with N=1, in general fNT); "|H(f)|²（無因次）" = |H(f)|² (dimensionless); "三種 jitter 核的正規化 |H(f)|²：TIE、N-period、cycle-to-cycle" = normalized |H(f)|² of the three jitter kernels: TIE, N-period, cycle-to-cycle.

- **Formulas**: kernel (a) $=1$, kernel (b) $=4\sin^2(\pi fT)$ (with $N=1$), kernel (c)
  $=16\sin^4(\pi fT)$ — exactly the integrands of the three boxed formulas above, plotted
  against the normalized frequency $fT$ (dimensionless) on the horizontal axis.
- **script / function**: `simulations/lab_24_jitter_kernels.py` (`make_figure_kernels`),
  writing `static/figures/jitter_kernel_shapes.png`.
- **How to read it**: on the log-log plot the three curves peak at $1$ (TIE — unfiltered,
  constant), $4$ (period — peaking at $fT=0.5$), and $16$ (c2c — also peaking at
  $fT=0.5$, marked on the plot with horizontal dashed lines and numbers); the low-frequency
  dashed lines are the $f\to0$ asymptotes $(2\pi fT)^2$ (period) and $(2\pi fT)^4$ (c2c) —
  a first-order and a second-order high-pass, respectively: the higher the differencing
  order, the less close-in phase noise gets counted. This single figure condenses the full
  shape information of this section's three boxed formulas for later reference whenever a
  kernel's shape matters.
...
- **Units (important — a frequent source of confusion)**: this $\kappa$ is the **phase version**,
  ...
  There is **no $\omega_0$** inside it (the Eq.(12) printed on [P2] p.793 **does** carry $\omega_0$ in its denominator — it is the time version below; re-verified in v11). The **time version** follows from [P2] Eq.(10), p.793,
...
## Paper-native derivation: [P2] Appendix A (verbatim reconciliation, now a standalone page)

The verbatim reconciliation of [P2] Eq.(40)-(51) now lives on
[p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation) — the verbatim
transcription and factor-by-factor reconciliation of [P2] **Appendix A, "Relationship
Between Jitter and Phase Noise"** (p.802-803): the white-noise time-domain autocorrelation
route Eq.(40)-(44) (= Section 4.2 above, converging on the main text's Eq.(11)) and the
autocorrelation + Khinchin route Eq.(45)-(51) (= Route A above, plus two white-noise
special-case corollaries), with a step-by-step mapping table, an honest account of what
each route skips, and a Python replay computing both routes for the same oscillator and
landing on the same $\kappa^2\Delta T$. Steps 3 and 4 above are this site's own derivation;
that page shows the paper's two routes interlock exactly with these.
...
two-regime log-log plot of Fig. 16. **Site convention**: this $\zeta$ (throughout this section and page)
is [P2] Fig.16's symbol, not the PLL damping ratio $\zeta$ (see pll_noise_budget and related pages).
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles/suptitle read: "(a) canonical 5 GHz：白噪+flicker FM，斜率 …" = (a) canonical 5 GHz: white + flicker FM, slope …; "(b) [P2] Fig.16（osc 12, 2.8 GHz）之 κ/ζ 漸近線重繪" = (b) [P2] Fig. 16 (osc 12, 2.8 GHz) κ/ζ asymptote redrawn; "lab_24 Part 5：兩段式 jitter 成長 …（[P2] Fig.16, Eq.(8)/(9)）" = lab_24 Part 5: two-regime jitter growth … ([P2] Fig. 16, Eq. (8)/(9)).
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Labels/titles read: "fT（正規化頻率, 無因次）" / "|H(f)|²（無因次）" = fT (normalized frequency, dimensionless) / |H(f)|² (dimensionless), title "三種 jitter 的權重核" = weighting kernels of the three jitter types; "N（相隔週期數）" = N (number of periods apart), title "白噪 FM 隨機漫步：κ=0.354 rad/√s" = white-FM random walk: κ = 0.354 rad/√s; "機率密度 [1/fs]" = probability density [1/fs], title "period vs c2c（理論 σ_c2c=√2 σ_P）" = period vs. cycle-to-cycle (theory σ_c2c = √2 σ_P); suptitle "lab_24：jitter 核 Monte-Carlo 驗證（f0=5 GHz, Γrms=0.5, qmax=1 pC, Si=1e-24 A²/Hz）" = lab_24: Monte-Carlo verification of the jitter kernels (f0 = 5 GHz, Γrms = 0.5, qmax = 1 pC, Si = 1e-24 A²/Hz).
...
- jitter ← phase spectrum (autocorrelation + Khinchin route): [P2] Eq.(46)–(49), p.803
  ...
  column of p.803; verbatim transcription and factor-by-factor reconciliation in
  [p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation)); white-noise special case
  ...
  Eq.(51) $\sigma_{CTC}=\tfrac{f}{f_0^{1.5}}\cdot10^{-\mathcal{L}\{\Delta f\}/20}$ (as printed — the numerator reads $f$; the dimensions of Eq.(50)×$\sqrt T$ show it should read the offset frequency $\Delta f$, the $\Delta$ being a printing omission, see A.2 of [p2_appendix_a_reconciliation](/99_appendix/p2_appendix_a_reconciliation)) — **the printed equation has no $\sqrt2$**: its $\sigma_{CTC}=\kappa\sqrt{T}$ is "the accumulation over one period" (i.e., this page's $\sigma_P$); with the "adjacent-period difference" definition (this page's $16\sin^4$ kernel) multiply by another $\sqrt2$. Both definitions coexist in the literature; this page names them separately.)
