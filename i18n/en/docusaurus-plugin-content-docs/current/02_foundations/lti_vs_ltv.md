...
> 1. LTI's $h(t-\tau)$ versus LTV's $h_\phi(t,\tau)$ — which extra independent variable makes the difference?
> 2. Why does the same impulse produce completely different phase effects at the waveform peak versus at a zero crossing — $0$ vs $-1$ mrad for $\Delta q=1$ fC, $q_{max}=1$ pC?
> 3. Why is this time-variance exactly the reason the ISF exists?
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. The left-panel title reads: "LTI: h(t,τ)=h(t-τ) — 形狀相同，只是平移" = LTI: h(t,τ)=h(t-τ) — same shape, just shifted.
