...
> 1. Why is "phase" a tangential coordinate on the limit cycle, rather than corresponding to some absolute time reference?
> 2. Why is a radial (amplitude) deviation pulled back while a tangential (phase) deviation is left permanently?
> 3. How does this geometric picture turn directly into phase noise and timing jitter?
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Annotations/title read: "impulse @ peak → 只有 ΔA, 無 Δφ (Γ≈0)" = only ΔA, no Δφ (Γ≈0), "impulse @ zero crossing → 最大 Δφ, 無 ΔA (|Γ| 最大)" = maximum Δφ, no ΔA (|Γ| is maximum), title "同樣大小的 impulse，注入相位不同 → 效果完全不同 (LTV 本質)" = same-size impulses injected at different phases → completely different effects (the essence of LTV).
