...
The three problems below run the full chain and the weighting kernel once each, and the third demonstrates power-summing independent noise
sources. Format: problem → step-by-step substitution (with units) → result → dimension check → one-line Python verification (using `simulations/common/`).
...
### Example E (adding two noise sources): independent sources add in $S_\phi$ (power)

> **Problem**: a real oscillator is never driven by just one noise source. This example demonstrates **multi-source
> superposition** — two independent white-noise sources hitting the same ideal-LC node at once. How do we combine them
> into a total phase noise? Reuse the oscillator parameters from
> [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) Example 1 ($f_0=5$ GHz, $\Delta f=1$ MHz,
> $q_{max}=1$ pC). Superpose two statistically independent white-noise current sources on the node:
...
The two are **distinct physical noise sources and are independent** (uncorrelated), so the question is: what is the
combined $\mathcal{L}_{total}(1\text{MHz})$? The formula is the same [P1] Eq.(21) shared with
...
**Step 1: compute Device A alone (same recipe as [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) Example 1, using [P1] Eq.(21)).**
...
so $\mathcal{L}_A=10\log_{10}(1.583\times10^{-15})\approx-148.0\ \text{dBc/Hz}$ (matches
[white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) Example 1 — same parameters).
...
just as in [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise) Examples 1 and 2, and both
reduce to $\text{s}$ (per-Hz, see that page's Example 1 dimension check) — only quantities with the same dimension
may be added; the sum is still $\text{s}$, and after $10\log_{10}$ it reads as dBc/Hz ✓.
...
in [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise): the constant convention does
not affect scaling/relative relationships. The full rule for multi-source superposition and the derivation of
the cyclostationary $\Gamma_{eff}$ are in [effective_isf](/03_isf_core_theory/effective_isf); full libraries:
`simulations/common/noise_utils.py`, `simulations/common/isf_utils.py`.)
...
- **Example E**: independent noise sources add in $S_\phi$ (linear power), never in dBc/Hz; two equal-power sources
  combine to only $+3.0$ dB — comparing $\Gamma_{rms}$ magnitudes alone will not tell you that.
