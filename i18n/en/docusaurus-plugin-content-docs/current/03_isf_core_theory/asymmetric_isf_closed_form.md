...
<NumericQuiz
  prompt="Same ring with N = 5, η = 1, device 1/f corner f₁/f = 1 MHz. The waveform asymmetry ratio worsens slightly from A = 1.5 to A = 2. By what factor does the 1/f³ corner grow? (Hint: this ratio is independent of the [P1]/[P2] factor-2 convention)"
  answer={2.33}
  tol={0.02}
  unit="×"
  hint="Only (1−A)²/(1−A+A²) in Eq.(57) depends on A: A=1.5 gives 0.25/1.75 = 1/7; work out the A=2 case yourself with the same formula."
  solutionNote="At A=2, (1−A)²/(1−A+A²) = 1/3; the ratio = (1/3)/(1/7) = 7/3 ≈ 2.33 (much gentler than the ×4.0 seen when A worsens all the way to 3 — the corner is less sensitive to A near A=1)."
/>
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/titles read: "注入相位 x/2π（x=ω0τ [rad]）" = injection phase x/2π (x=ω0τ [rad]); "Γ(x) [無因次]" = Γ(x) [dimensionless]; "(a) [P2] Fig.18 非對稱三角 ISF（N=5, η=1）\n正葉 1/f'_rise、負葉 1/f'_fall，虛線 = Γ_dc（Eq.(56)）" = (a) [P2] Fig. 18 asymmetric triangular ISF (N=5, η=1); positive lobe 1/f'_rise, negative lobe 1/f'_fall, dashed = Γ_dc (Eq. (56)); "不對稱比 A=f'_rise/f'_fall [無因次]（log 軸）" = asymmetry ratio A=f'_rise/f'_fall [dimensionless] (log axis); "(b) Eq.(57)：對稱點 A=1 的 V 形谷底\n（[P2] Fig.17 對稱電壓碗底的解析版；對 A→1/A 對稱）" = (b) Eq. (57): the V-shaped valley at the symmetric point A=1 (the analytic counterpart of [P2] Fig. 17's symmetric voltage bowl; symmetric under A→1/A); "級數 N [無因次]（log 軸）" = number of stages N [dimensionless] (log axis); "(c) Eq.(57)：固定 A 下 corner ∝ 1/N\n（級數越少 → 1/f³ corner 越高；[P2] p.803 原句）" = (c) Eq. (57): at fixed A the corner ∝ 1/N (fewer stages → higher 1/f³ corner; verbatim from [P2] p.803).
