...
- **Link to [P2]**: this general formula is the engine behind the ring paper's accumulated-jitter law — [P2] Eq.(8), p.792's
  $\sigma_{\Delta\phi}=\kappa\sqrt{\Delta t}$ is exactly the special case "white FM → linear variance" (the phase version
  $\kappa=\omega_0\kappa_t$ = the square root of [P2] Eq.(11), p.793; the printed Eq.(12), p.793 gives the time version $\kappa_t$, with $\omega_0$ in the denominator).
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles/labels read: "同一個 L(10 kHz)，兩種線形：Lorentzian vs 近高斯" = the same L(10 kHz), two lineshapes: Lorentzian vs. near-Gaussian; "|Δf| / HWHM（各自線寬歸一）" = |Δf| / HWHM (normalized by each line's own linewidth); "相對峰值 [dB]" = relative peak [dB]; "線核形狀：white 比值 …（Lorentzian 3.00）；flicker …（Gaussian 1.82）" = line-core shape: white ratio … (Lorentzian 3.00); flicker … (Gaussian 1.82); "相位增量方差：white 線性成長；flicker τ²×log 成長" = phase-increment variance: white grows linearly; flicker grows as τ²×log; "10 kHz 交點（同 S_φ → 同 L）" = the 10 kHz crossing (same S_φ → same L); "S_φ(f) [rad²/Hz]（單邊）" = S_φ(f) [rad²/Hz] (one-sided); "有限觀察時間的 S_φ：1/f² vs 1/f³，10 kHz 交會" = S_φ under finite observation time: 1/f² vs. 1/f³, crossing at 10 kHz.
...
- **Increment language**: [P2] Eq.(8), p.792 ($\sigma=\kappa\sqrt{\Delta t}$, increments; this page uses the phase version),
  Eq.(11)–(12), p.793 (phase version $\kappa=\frac{\Gamma_{rms}}{q_{max}}\sqrt{\tfrac12\overline{i_n^2}/\Delta f}$ = square root of Eq.(11);
  the printed Eq.(12) is the time version $\kappa_t=\kappa/\omega_0$, with $\omega_0$ in the denominator, re-verified in v11) — precisely the engineering embodiment of this page's "the PSD does not exist, the increments do".
