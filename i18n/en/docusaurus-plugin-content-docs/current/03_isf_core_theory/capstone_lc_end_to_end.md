...
> Prerequisites: [diffusion_dictionary](/03_isf_core_theory/diffusion_dictionary) | Next: [final_exam](/04_simulation_labs/final_exam)

> **What this page answers**:
> 1. Starting from the state equations of an ideal parallel LC, why must Floquet linearization always produce a phase direction with eigenvalue $\lambda_1=0$?
> 2. How does the geometric projection give $\Gamma(\theta)=-\sin\theta$ and $\Gamma_{rms}=1/\sqrt2$ (Parseval)? Plugged into [P1] Eq.(21), why does that come out to $-145$ dBc/Hz @ 1 MHz (canonical example B's $\Gamma_{rms}=0.5$ gives $-148$, a 3 dB difference)?
> 3. How does the Lorentzian tame the $1/f^2$ divergence as $\Delta\omega\to0$ — where does the true-LC ($D=0.125$ rad²/s) 3-dB linewidth $\Delta f_{3\mathrm{dB}}=D/\pi=39.8$ mHz come from?
> 4. Integrating a measured $-100$ dBc/Hz @ 1 MHz from 1 to 100 MHz gives $\sigma_t=447.9$ fs — why? And how does that turn into the 6.3% UI eaten out of a 10 Gb/s eye at BER $10^{-12}$?
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Panel (b) title reads: "(b) BER bathtub：jitter 越大，可用取樣窗（eye）越窄" = (b) BER bathtub: the larger the jitter, the narrower the usable sampling window (eye).
