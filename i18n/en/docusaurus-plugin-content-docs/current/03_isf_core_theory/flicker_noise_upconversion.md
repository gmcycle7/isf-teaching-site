...
> **Translator's note**: all equations are preserved byte-for-byte from the original. Chinese labels appearing inside math read as follows: flicker 只剩 DC = flicker leaves only the DC term, 機制（積分器） = mechanism (integrator), flicker 機制 = flicker mechanism.

# Flicker noise upconversion into 1/f³ phase noise

> Prerequisites: [lorentzian_linewidth](/03_isf_core_theory/lorentzian_linewidth) | Next: [effective_isf](/03_isf_core_theory/effective_isf)
>
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Title reads: "c0≠0 才會把 1/f noise upconvert 到 close-in phase noise" = only c0≠0 upconverts 1/f noise into close-in phase noise.
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Title reads: "Flicker upconversion: ISF 對稱性 (c0) 決定 close-in 1/f³" = Flicker upconversion: the ISF's symmetry (c0) determines the close-in 1/f³ term.

**Limits (honesty note)** — [P2] makes the following three points in Sec. VII Design Implications (p.798, original text);
the "more linear loads" point below is also independently made by [P1] (Sec. IV, p.189) — see below.
...
- **More linear loads help**: [P2]'s own text (p.798) states, in its own words, that using more linear loads (e.g. resistors or
  linearized MOS devices) reduces 1/f noise upconversion and substrate/supply coupling (citing [20] Maneatis & Horowitz 1993).
  [P1] (Sec. IV, p.189) independently traces this back to prior observations in the context of supply-noise rejection —
  [15] Maneatis 1993 and [16] Yang/Farjad-Rad/Horowitz 1997 (more linear loads reduce the effect of supply noise on timing
  jitter) — and then adds a new point on top of that: the same measure also makes the waveform more symmetric and lowers
  the corner for low-frequency-noise upconversion into phase noise. In short, both papers independently arrive at — and
  corroborate — the same design recommendation; it is not unique to either one.
...
Core Python (full script: `simulations/lab_07_flicker_noise.py`; **site convention**: the `alpha` parameter of
`gamma_asymmetric` below is the DC offset of the toy ISF $\Gamma=\cos\theta+\alpha$ ($c_0=2\alpha$), unrelated
to [P1]'s NMF $\alpha(\omega_0t)$):
