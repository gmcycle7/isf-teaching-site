...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels read: "state v (電容電壓, normalized)" = state v (capacitor voltage, normalized); "state w (∝ 電感電流, normalized)" = state w (∝ inductor current, normalized).
