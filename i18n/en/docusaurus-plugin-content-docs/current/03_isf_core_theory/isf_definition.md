...
> Prerequisites: [impulse_to_phase_shift](/03_isf_core_theory/impulse_to_phase_shift) | Next: [convolution_derivation](/03_isf_core_theory/convolution_derivation)
...
2. **charge → voltage step**: on the node capacitance $C_{node}$ the voltage jumps instantaneously by $\Delta V=\Delta q/C_{node}$ ([P1] Eq.(9), p.181). Units $[\text{C}]/[\text{F}]=[\text{V}]$ ✓.
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Panel titles read: "(a) LC 波形與其 ISF" = (a) LC waveform and its ISF; "(b) Δφ 與注入電荷成正比 (small-signal)" = (b) Δφ is proportional to the injected charge (small-signal); "(c) zero-crossing 注入 → 相位跳變" = (c) zero-crossing injection → a phase jump.
...
| **[P2]** Hajimiri–Limotyrakis–Lee 1999 | $\Gamma(\omega_0\tau)$ | jitter/phase noise of ring oscillators | the same $\Gamma$; emphasizes the $\Gamma_{rms}\propto N^{-3/2}$ scaling ([P2] Eq.(16), p.794; v7 re-verified: the radical covers only the constant; the text's 4/N^{1.5}@η=0.75 and App.B Eq.(55) triple-confirm it. An earlier v3 pass misread this as N^{-3/4}) | high (statement and scaling both verified) |
| **[P3]** Hong–Hajimiri 2019 Part I | $\Gamma(\theta+\phi)$ | injection locking/pulling (generalized Adler) | **the same $\Gamma$**, moved to the injection context: $\frac{d\phi}{dt}=\Delta\omega+\frac{1}{q_{max}}\langle\Gamma(\theta+\phi)\,i_{inj}(\theta)\rangle$, $\Delta\omega\equiv\omega_0-\omega_{inj}$ ([P3] Eq.(30), p.2113, with a **plus** sign in front of the averaged term; [P3]'s $\tilde\Gamma=\Gamma/q_{max}$ (Eq.(26), p.2113) has the **same sign** as this site's $\Gamma=-\sin\theta$: [P4] Eq.(24), p.2128 gives $\tilde\Gamma(\varphi)=-\sin\varphi/q_{max,0}$ and Eq.(26) gives $\tilde\Gamma_1=(1/q_{max,0})\angle90^\circ$. This site consistently uses $\Delta\omega\equiv\omega_0-\omega_{inj}$, which differs from the $\Delta\omega\equiv\omega_{inj}-\omega_0$ of [P3] Fig. 5 by an overall sign) | high (checked against the original PDF; v11 correction: the earlier version wrote a minus sign and claimed an "opposite sign convention") |
| **[P4]** Hong–Hajimiri 2019 Part II | $\Delta(\phi)$ (APF) | amplitude modulation (amplitude domain) | **the amplitude version**: projects the impulse onto the **radial** rather than the tangential direction; units $\text{A}^{-1}$; in the ideal LC the ISF and APF are in quadrature ([P4] Eq.(26), p.2128) | ✓ (APF=[P4] Eq.(19), Fig. 5, p.2126, verified) |
...
- Across the papers: [P1][P2] use $\Gamma$ for phase noise; [P3] applies the same $\Gamma$ to injection; [P4]'s APF $\Delta$ is the amplitude counterpart; [P5] is unrelated to the ISF.
