...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles/labels read: "[P1] Fig. 29 概念復刻：n 階振盪器的 state-space 軌跡\n（van der Pol μ=2.0 實算：細線＝踢一下後真的重新收斂）" = [P1] Fig. 29 conceptual replica: state-space trajectory of an n-th order oscillator (an actually-integrated van der Pol μ=2.0: thin line = the genuinely re-converging trajectory after a kick); "x (radians)（θ=0 對齊波形谷值）" = x (radians) (θ=0 aligned to the waveform minimum); "[P1] Fig. 30 概念復刻：Calculation of Impulse Sensitivity Function\n（van der Pol μ=0.2；原圖振盪器未載明）" = [P1] Fig. 30 conceptual replica: Calculation of Impulse Sensitivity Function (van der Pol μ=0.2; the original figure does not specify the oscillator).
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/annotations read: "注入相位 θ/2π（θ=0 對齊波形峰值，無因次）" = injection phase θ/2π (θ=0 aligned to the waveform peak, dimensionless); "Γ(θ)（無因次，qmax=A）" = Γ(θ) (dimensionless, qmax=A); "近諧波：形狀吻合，但峰值附近差 O(μ)" = near-harmonic: shapes match, but they differ by O(μ) near the peak; "強非線性：closed form 失準" = strongly nonlinear: the closed form loses accuracy.
