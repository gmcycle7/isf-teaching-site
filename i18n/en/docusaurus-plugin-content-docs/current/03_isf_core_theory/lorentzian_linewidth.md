...
> Prerequisites: [rms_isf](/03_isf_core_theory/rms_isf) | Next: [flicker_noise_upconversion](/03_isf_core_theory/flicker_noise_upconversion)
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles read: "載波是 Lorentzian：近載波轉平（有限峰），1/f² 只是遠端漸近" = the carrier is Lorentzian: it flattens near the carrier (a finite peak), 1/f² is only the far-out asymptote; "相位是 random walk：方差線性成長 → 載波自相關指數衰減" = phase is a random walk: variance grows linearly → the carrier autocorrelation decays exponentially.
...
<NumericQuiz
  prompt="Keeping the explorer at its default settings (L(10 kHz) = −71 dBc/Hz, white FM, RBW = 100 Hz), what FWHM does the panel measure (after RBW convolution)?"
  answer={130}
  tol={0.05}
  unit="Hz"
  hint="First get the true FWHM = D/π (D comes from converting L(10 kHz), Steps 3/5). Then note RBW=100 Hz is already much wider than the true FWHM, so the measured linewidth gets smeared by the instrument's own resolution to roughly the RBW's own scale."
  solutionNote="true FWHM≈49.9 Hz, but RBW=100 Hz is far wider than that, so the convolved measurement smears out to ≈130 Hz — once RBW exceeds the true linewidth, what you read is essentially the instrument's own resolution shape, not the true lineshape."
/>

<details>
<summary><strong>Verification: reproducing the panel's measured FWHM</strong> (mirrors LineshapeExplorer.js's whiteLineshape / buildGrid / gaussKernel / convolveSame / fwhmOfCurve)</summary>
...
# widget defaults: L_dbc=-71.0, noiseType='white', logRbw=2.0 -> RBW=100 Hz (LineshapeExplorer.js:196-198)
L_dbc = -71.0
rbw = 10 ** 2.0

Llin = 10 ** (L_dbc / 10)
Sphi = 2 * Llin                                             # time-domain /2 convention
