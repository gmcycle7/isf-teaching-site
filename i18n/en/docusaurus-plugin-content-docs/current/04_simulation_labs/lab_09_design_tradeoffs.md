...
> **Translator's note**: all equations are preserved byte-for-byte from the original. Chinese labels appearing inside math read as follows: 固定 = holding … fixed, 積分範圍 = integration range.
