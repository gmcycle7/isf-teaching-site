...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Legend/annotation/title read: "有 phase noise（裙帶 sidebands）" = with phase noise (skirt / sidebands), "理想載波" = ideal carrier, title "Phase noise 把載波塗成裙帶：時域 PM → RF 頻譜（[P1] Fig.8 的圖像）" = phase noise smears the carrier into a skirt: time-domain PM → RF spectrum ([P1] Fig. 8's picture).
