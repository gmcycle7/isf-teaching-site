...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Legend/title read: "量得 σ=… fs (理論 … fs)" = measured σ=… fs (theory … fs), title "RJ 是高斯，且 $\sigma_{\Delta t}=\sigma_{edge}\sqrt{\Delta N}$" = RJ is Gaussian, and $\sigma_{\Delta t}=\sigma_{edge}\sqrt{\Delta N}$.
