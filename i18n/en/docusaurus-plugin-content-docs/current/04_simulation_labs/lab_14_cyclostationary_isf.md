...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles read: "(a) ISF 與兩種 NMF α(θ)" = (a) the ISF and two NMFs α(θ); "(b) 同一顆 device，注入相位決定 Γeff,rms (stationary=…)" = (b) the same device: the injection phase determines Γeff,rms (stationary=…).
