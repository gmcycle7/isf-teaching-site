...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles read: "(b) 對稱波形／低 Γrms → PN↓" = (b) a symmetric waveform / low Γrms → lower PN; "(c) 固定 f0,P：PN 幾乎與 N 無關（[P2]，常數待查）" = (c) at fixed f0, P: PN is nearly independent of N ([P2], the constant is unverified).
