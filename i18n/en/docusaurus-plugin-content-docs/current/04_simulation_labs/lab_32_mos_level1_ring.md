...
- In code the three regions are implemented as a single clamped expression:
  ...
  $V_{DE}=\min(V_{DS},V_{ov})$, $I_D=\beta\,(V_{ov}-V_{DE}/2)\,V_{DE}$ (**site convention**: here
  $\beta=\mu C_{ox}W/L=k'W/L$ is the MOS transconductance parameter — a distinct symbol from the
  injection-locking realignment factor $\beta$ and the FM modulation index $\beta$) — **pointwise equal**
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/titles read: "t-t0 [ps]（t0 = 節點1上升緣過 VDD/2）" = t-t0 [ps] (t0 = node 1's rising edge crossing VDD/2); "節點電壓 [V]" = node voltage [V]; "(a) Level-1 方程級 3 級 ring 波形（一週期）\n f0=…GHz, T=…ps（非 SPICE/BSIM）" = (a) equation-level-1 3-stage ring waveform (one period); f0=…GHz, T=…ps (not SPICE/BSIM); "注入相位 θ/2π（θ=0: 節點1上升緣）" = injection phase θ/2π (θ=0: node 1's rising edge); "Γ(θ) [無因次]" = Γ(θ) [dimensionless]; "(b) 萃取 ISF：能量集中在自身 transition\n Γrms=…, c0=…（藍=上升窗, 紅=下降窗, …% 能量）" = (b) extracted ISF: energy concentrates at its own transition; Γrms=…, c0=… (blue = rising window, red = falling window, …% of the energy); "諧波序 n" = harmonic index n; "(c) ISF 傅立葉係數（[P1] Eq.(12)）\n c0=… 小 → 1/f³ 上轉弱（[P1] Eq.(23)(24)）" = (c) ISF Fourier coefficients ([P1] Eq. (12)); small c0 → weak 1/f³ upconversion ([P1] Eq. (23)(24)).
