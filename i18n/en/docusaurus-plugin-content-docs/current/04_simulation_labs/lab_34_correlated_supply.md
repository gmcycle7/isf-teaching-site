...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles read: "(a) N=5 各級 ISF（灰）與總和（紅）" = (a) the per-stage ISFs (gray) and their sum (red) for N=5; "(b) 只有 n≡0 (mod 5) 存活" = (b) only n≡0 (mod 5) survives; "(c) 共同注入只在 n=0,5,10 有響應" = (c) a common injection only responds at n=0, 5, 10.
