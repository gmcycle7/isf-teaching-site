...
**Site convention**: on this page, $\Delta\omega$ is the detuning, not the "offset frequency" in the notation
table; after this point, offset frequency is always written $\omega$.
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/titles read: "失諧比 r=Δω/ωL" = detuning ratio r=Δω/ωL; "捕獲時間 ωL Tacq（無因次）" = capture time ωL·Tacq (dimensionless); "(a) 鎖定捕獲：settle 率＝pull-in 頻率 ωc（[P3] Eq.(40)）\n鎖定邊緣 r→1：ωc→0，捕獲時間發散（臨界慢化）" = (a) lock acquisition: the settle rate = the pull-in frequency ωc ([P3] Eq. (40)); at the lock edge r→1: ωc→0, capture time diverges (critical slowing down); "障壁／雜訊比 ΔU/D（D＝慣例乙擴散常數）" = barrier-to-noise ratio ΔU/D (D = the site's convention-B diffusion constant); "cycle-slip 率 ν/ωL（每單位 ωL t）" = cycle-slip rate ν/ωL (per unit ωL·t); "(b) 雜訊誘發 cycle slips（r=0.8，ΔU=0.1704 ωL）\nlog-linear 直線＝Arrhenius 型 e^(-ΔU/D)；斜率＝障壁高度" = (b) noise-induced cycle slips (r=0.8, ΔU=0.1704 ωL); a log-linear line = Arrhenius-type e^(-ΔU/D); slope = barrier height; "單一 walker（ΔU/D=5）：階梯＝slips" = a single walker (ΔU/D=5): each step = a slip; suptitle "鎖定捕獲暫態與 noise-induced cycle slips：同一條 Adler 方程、同一個根號 ωc=√(ωL²-Δω²)（[P3] Eq.(38)–(40)；[P4] Eq.(31)–(32)；Kramers＝外部文獻）" = lock-acquisition transient and noise-induced cycle slips: the same Adler equation, the same radical ωc=√(ωL²-Δω²) ([P3] Eq. (38)-(40); [P4] Eq. (31)-(32); Kramers is an external-literature result).
