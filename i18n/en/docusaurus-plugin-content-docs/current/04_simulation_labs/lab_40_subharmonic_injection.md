...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image; it bakes in many CJK axis labels, titles and annotations. Highlights: "倍頻比 N（fref=f0/N）" = division ratio N (fref=f0/N); "半鎖定範圍 fL [MHz]" = half lock range fL [MHz]; "(a) 脈衝注入 lock range ∝ 1/N（固定 qinj=50 fC）" = (a) pulse-injection lock range ∝ 1/N (fixed qinj=50 fC); "脈衝寬度 τp/T0（T0=200 ps）" = pulse width τp/T0 (T0=200 ps); "(b) 脈衝越寬、諧波越少、lock range 越小" = (b) the wider the pulse, the fewer the harmonics, the smaller the lock range; "(c) 同樣 Irms=250 μA：脈衝串鎖定、純正弦鎖不住（一階）" = (c) at the same Irms=250 μA: a pulse train locks, a pure sinusoid does not (first order); "預測 β（無因次）" / "量測 β" = predicted β (dimensionless) / measured β; "(d) 重新對齊因子 β：ODE 步階響應 vs 一階預測" = (d) the realignment factor β: ODE step response vs. first-order prediction; "offset 頻率 f [Hz]" = offset frequency f [Hz]; "(e) 鎖定相位雜訊＝一階離散高通整形（LC，N=20，Δω=0）" = (e) locked phase noise = first-order discrete high-pass shaping (LC, N=20, Δω=0); "倍頻比 N（固定 qinj → 固定 β=0.0498）" = division ratio N (fixed qinj → fixed β=0.0498); "輸出 rms jitter σt [fs]" = output rms jitter σt [fs]; "(f) jitter ∝ √N（迴路角落 fc=βfref/2π∝1/N）" = (f) jitter ∝ √N (loop corner fc=βfref/2π∝1/N); suptitle "次諧波（×N）脈衝注入鎖定：impulse-train 映射 vs 未平均時間同步 ODE" = subharmonic (÷N) pulse-injection locking: impulse-train map vs. un-averaged time-synchronous ODE.
