...
> **Verified (corrected in v11 after re-reading the PDF)**: [P2] Eq.(12), p.793 as printed is $\kappa=\dfrac{\Gamma_{rms}}{q_{max}\,\omega_0}\sqrt{\tfrac12\tfrac{\overline{i_n^2}}{\Delta f}}$ — the denominator **contains $\omega_0$**, the units are $\sqrt{\text{s}}$, and it is the **timing** jitter constant $\kappa_t$ of Eq.(8) $\sigma_{\Delta T}=\kappa\sqrt{\Delta T}$ (assembled from Eq.(10) $\sigma_{\Delta\phi}=\omega_0\sigma_{\Delta T}$ and Eq.(11) $\sigma_{\Delta\phi}^2=\Gamma_{rms}^2(\overline{i_n^2}/\Delta f)\,\Delta T/(2q_{max}^2)$; verified verbatim against the original PDF rendering). The $\kappa_\phi=\dfrac{\Gamma_{rms}}{q_{max}}\sqrt{\tfrac12\tfrac{\overline{i_n^2}}{\Delta f}}$ (rad/$\sqrt{\text{s}}$) of this site's [diffusion_dictionary](/03_isf_core_theory/diffusion_dictionary) is the **phase version** of the same thing: $\kappa_\phi=\omega_0\kappa_t$, numerically $\kappa_t=\kappa_\phi/(2\pi f_0)$. v4–v5 had recorded "Eq.(12) has no $\omega_0$" as verified; corrected in v11, with every number on this site unchanged.
> Both versions share the same $\Gamma_{rms}^2/q_{max}^2$ ratio with phase noise (claim C6).
...
$\kappa_{min}$ grows by $\times\sqrt3\approx1.732$. (**Convention flag (corrected in v11)**: Eq.(34) is collected from $2N\times$Eq.(6), and the printed [P2] Eq.(6), p.792 has denominator $8\pi^2f_{off}^2=2\Delta\omega^2$ — this site's $\mathcal{L}_{/2}$ family, **3 dB above** the $/4$ SSB family of [P1] Eq.(21) with its $4\Delta\omega^2$. So the absolute values $-82.7$/$-78.0$ dBc/Hz are **$/2$-family** numbers; in the [P1] Eq.(21) bookkeeping they read $-85.7$/$-81.0$ dBc/Hz. $\Delta\mathcal{L}$ is a difference, **identical under both conventions**.)
...
> **Convention flag (the two outfits of κ, consistent site-wide)**: the printed [P2] Eq.(12), $\kappa=\dfrac{\Gamma_{rms}}{q_{max}\omega_0}\sqrt{\tfrac12\dfrac{\overline{i_n^2}}{\Delta f}}$, has $\omega_0$ in the denominator and units of $\sqrt{\text{s}}$ (the time-domain $\kappa_t$, matching the $\sigma_{\Delta T}$ of Eq.(8)); the $\kappa_\phi^2=\Gamma_{rms}^2S_i/(2q_{max}^2)$ of this site's [diffusion_dictionary](/03_isf_core_theory/diffusion_dictionary) is the phase version of Eq.(11) (rad²/s) — the same physical quantity, differing only by the $\omega_0$ of Eq.(10): $\kappa_t=\kappa_\phi/\omega_0$ (here $\omega_0=2\pi\times2.81$ GHz). Since v11 this page and the dictionary page say the same thing.
