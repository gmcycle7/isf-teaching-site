...
| Large-injection half lock range | $\omega_L\equiv\omega_{L0}/\sqrt{1-a^2}$ | Eq.(9) = Eq.(23) at $\beta=90^\circ$, where $\beta\equiv\angle\tilde\Gamma_1-\angle\Delta_1$ (see the Eq.(23) derivation below) | Whenever this page writes $\omega_L$ it means the **APF-corrected** value; ISF-only is always $\omega_{L0}$; **site convention**: this $\beta$ is the ISF–APF phase difference, unrelated to the realignment factor $\beta$ on subharmonic_injection, the FM modulation index $\beta$, or the MOS $\beta$ — four distinct symbols |
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/titles read: "ωL0 t（無因次時間）" = ωL0·t (dimensionless time); "|θ̂(t)|/|θ̂(0)|（歸一化相位偏差）" = |θ̂(t)|/|θ̂(0)| (normalized phase deviation); "(a) 鎖定捕獲：精確 tanh 解與 APF 修正的 pull-in 率" = (a) lock acquisition: the exact tanh solution vs. the APF-corrected pull-in rate; "θ(t) [deg]（紫）" = θ(t) [deg] (purple); "t [ns]（f0=5 GHz、Iinj=1.5 mA、Q=10）" = t [ns] (f0=5 GHz, Iinj=1.5 mA, Q=10); "振幅 Vosc/Vosc,0=1+A(t)" = amplitude Vosc/Vosc,0=1+A(t); "(b) 捕獲期間的振幅暫態：dip → overshoot → settle（θ(0)=−2 rad）" = (b) the amplitude transient during capture: dip → overshoot → settle (θ(0)=−2 rad); "相對相位 θ [deg]" = relative phase θ [deg]; "鎖定失諧 Δω/ωL0=sinθ/(1+a cosθ)" = locked detuning Δω/ωL0=sinθ/(1+a cosθ); "(c) 大注入 lock characteristic：ωL=ωL0/√(1−a²)（[P4] Eq.9/23）" = (c) the large-injection lock characteristic: ωL=ωL0/√(1−a²) ([P4] Eq. 9/23); "相對主線 (k=1) [dB]" = relative to the main line (k=1) [dB]; "(d) pulled 頻譜（Δω=2ωL）：APF 把 k=0 與 k=2 線抬高" = (d) the pulled spectrum (Δω=2ωL): APF raises the k=0 and k=2 lines; suptitle "lab_41：[P4] 大注入 LC 模型（ISF/(1+A)）的鎖定捕獲、振幅暫態與 pulling 頻譜" = lab_41: [P4]'s large-injection LC model (ISF/(1+A)) — lock acquisition, amplitude transient, and pulling spectrum.
