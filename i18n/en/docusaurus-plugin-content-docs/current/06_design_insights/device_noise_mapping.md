...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/title/annotation read: "電流雜訊 PSD Si(f) [A²/Hz, arb.]" = current-noise PSD Si(f) [A²/Hz, arb.]; "Device noise 的頻譜，與 ISF 諧波接收的三個頻段 (DC, f0, 2f0)" = the device-noise spectrum, and the three frequency bands the ISF's harmonics receive from (DC, f0, 2f0); "頻率 f / f0（下面 stem 的 n 對齊 n f0）" = frequency f / f0 (the stems below have n aligned to n·f0); "ISF 諧波增益 |cn|" = ISF harmonic gain |cn|; "載波 (carrier)" = carrier.
