...
| **PI quantization** (phase interpolator quantization) | A $b$-bit phase interpolator slices one UI into $2^b$ grid points; the sampled/output phase can only land on a grid point (derivation: [clock_chain_budget](/06_design_insights/clock_chain_budget) Rule 5) | Uniform distribution (width $UI/2^b$) | Grid count is fixed, $\mathrm{DJ}_{pp}=UI/2^b$ (nominal lower bound; real DNL makes it larger) |

Note that **DCD's PDF is inherently two Diracs** — the dual-Dirac model is **exact** for it;
the model's name and shape come precisely from this "worst-case shape". **PI quantization's PDF is a
uniform distribution** — neither the sinusoid's double horn nor DCD's two points — a third common DJ
shape; at $b=6$, $UI=40$ ps, $\mathrm{DJ}_{pp}=UI/2^6=0.625$ ps (full derivation and `# ->` verification
in [clock_chain_budget](/06_design_insights/clock_chain_budget) Rule 5).
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles read: "(a) TJ 的 PDF：dual-Dirac 的 Dirac 在真極值內側" = (a) the PDF of TJ: the dual-Dirac's Diracs sit inside the true extreme values; "(b) Q-scale 尾巴擬合 → 取出 μ 與 σ" = (b) a Q-scale tail fit extracts μ and σ; "(c) BER bathtub：DJ 把兩壁往內推，RJ 決定壁的斜率" = (c) BER bathtub: DJ pushes both walls inward, RJ sets the walls' slope.
...
<NumericQuiz
  prompt="Keeping the explorer at its default settings (σ = 1.0 ps, A = 2.0 ps), if you plug the 'true' peak-to-peak DJ_pp = 2A directly into the TJ(BER) extrapolation formula (instead of the more conservative fitted DJ_δδ), what is TJ@10⁻¹² roughly?"
  answer={18.07}
  tol={0.01}
  unit="ps"
  hint="TJ = DJ_pp + 2·Q⁻¹(10⁻¹²)·σ = 2A + 2×7.034×σ. This plugs DJ straight in at its peak value, unlike Step 8's fitted DJ_δδ=3.16 ps, which gives 17.65 ps — because DJ_δδ ≤ DJ_pp (see Step 7's honesty note)."
  solutionNote="TJ@1e-12 = 2×2.0 + 2×7.034×1.0 = 4.0 + 14.068 = 18.07 ps. That's slightly larger than the 17.65 ps from Step 8's fitted DJ_δδ — here we used the un-fitted peak DJ_pp, echoing Step 7's honesty note that DJ_δδ ≤ DJ_pp and the under-report is deliberate: plugging in the peak value directly is more conservative (it overestimates TJ)."
/>

<details>
<summary><strong>Verification: the peak-value TJ@1e-12 estimate</strong> (mirrors DualDiracFitter.js's defaults and the Q_INV_1E12 constant)</summary>

```python
# widget defaults: sigma_ps=1.0, A_ps=2.0 (DualDiracFitter.js:201-203);
# Q_INV_1E12 = 7.034 (DualDiracFitter.js constant, matches this page's Step 3).
sigma_ps = 1.0
A_ps = 2.0
Q_INV_1E12 = 7.034

DJ_pp_ps = 2 * A_ps                                   # true peak-to-peak DJ (not the fitted DJ_dd)
tj_ps = DJ_pp_ps + 2 * Q_INV_1E12 * sigma_ps          # TJ(BER) formula (Step 6), with DJ_pp in place of DJ_dd
...
- PI quantization DJ and the full derivation of why a DLL "does not accumulate" without an oscillator: [clock_chain_budget](/06_design_insights/clock_chain_budget) Rule 5
