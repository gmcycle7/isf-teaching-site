...
<NumericQuiz
  prompt="Try it yourself first: same family of formulas, but with Q changed from 20 to 40 (everything else unchanged: F=1+γ, γ=2/3, Γ_rms²=0.5, η_P=1, 300 K, [P1] /4 convention) — the ceiling FOM_max = ? (answer in dB)"
  answer={209.67}
  tol={0.01}
  unit="dB"
  hint="FOM_max = C_ref − 10log₁₀(F·Γ_rms²/(2Q²·η_P)), with C_ref=173.83 dB; doubling Q adds 6.02 dB via 10log₁₀(Q²)."
  solutionNote="At Q=40, the bracket = (5/3×0.5)/(2×1600) ≈ 2.604×10⁻⁴ → FOM_max ≈ 173.83+35.84 ≈ 209.67 dB (6.02 dB above the Q=20 answer of 203.65 dB — exactly the 20log₁₀2 from doubling Q)."
/>
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/titles/legend read: 溫度 T (K) = Temperature T (K), FOM 上限 (dB) = FOM ceiling (dB), 品質因數 Q（無因次） = Quality factor Q (dimensionless), panel (a) title "(a) FOM 天花板家族 vs 溫度（各 $F_{eff}$ 一條線）" = (a) FOM ceiling family vs. temperature (one line per $F_{eff}$), panel (b) title "(b) 300 K：LC 天花板 vs Q、ring 天花板、參考線" = (b) 300 K: LC ceiling vs. Q, ring ceiling, reference line; legend entries: "$kT$ 參考線" = $kT$ reference line, "ring 天花板" = ring ceiling, "ring 例" = ring example, "LC 理想天花板" = LC ideal ceiling, "本站 ring 例" = this site's ring example, "本頁例 2 反推" = this page's Example 2, back-solved, "SSB /4 慣例" = SSB /4 convention, "時域 /2 慣例" = time-domain /2 convention.
...
- Turn this "how many dB below the ceiling" question around — compute the FOM a spec needs, pick a topology against
  the ceiling, then pin it all the way down to component values and bias — see
...
| Looking at FOM alone | Normalizes the power–noise trade-off | Area, tuning range (the FOM$_T$ variant covers this separately, defined at the end of [design_recipe](/06_design_insights/design_recipe)), supply pushing, yield are all outside its scope |
...
- The full flow from spec back to required FOM, topology choice, and component values: [design_recipe](/06_design_insights/design_recipe)
