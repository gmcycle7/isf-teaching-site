...
Define $\Delta\omega\equiv\omega_0-\omega_{inj}$ (detuning; **site convention**: on this page and the rest of the
injection-locking family, $\Delta\omega$ always means this detuning, not the "offset frequency" $\Delta\omega$
in the notation table — after this point, offset frequency is always written $\omega$) and
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Labels/titles read: "offset 頻率 f [Hz]" = offset frequency f [Hz]; "fc = … MHz\n(鎖定邊緣，抑制縮水)" = fc = … MHz (near the lock edge, the suppression shrinks); "(a) 鎖定振盪器自身相位雜訊被「高通整形」\n低於 ωc 壓平成 Sn/ωc²，高於 ωc 回到自由跑 1/f²" = (a) a locked oscillator's own phase noise is high-pass shaped: below ωc it flattens to Sn/ωc², above ωc it returns to the free-running 1/f²; "suppression 10log10(Slock/Sfree) [dB]" already English; "(b) 抑制量＝一階高通 |Hhp|²（與記帳慣例無關）\ncorner ωc=√(ωL²-Δω²)：越靠近鎖定邊緣越小" = (b) the suppression = a first-order high-pass |Hhp|² (independent of the SSB/time-domain accounting convention); the corner ωc=√(ωL²-Δω²) shrinks near the lock edge; suptitle "注入鎖定＝一階 PLL：自身雜訊高通、參考雜訊低通（此處參考乾淨）；[P3] Eq.(30)/(35)/(40) + 白噪 FM 驅動" = injection locking = a first-order PLL: own noise high-pass, reference noise low-pass (the reference is clean here); [P3] Eq. (30)/(35)/(40) plus white-FM driving noise.
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Labels/titles read: "時間 t [μs]" = time t [μs]; "θ/2π [cycle]" already mixed; "(a) 未鎖定：θ(t) 每拍滑一圈\n平緩段＝「準鎖定」逗留、陡段＝快速滑相（鋸齒狀）" = (a) unlocked: θ(t) slips a full cycle each beat; flat stretches = "quasi-lock" dwell, steep stretches = rapid phase slip (sawtooth); "瞬時頻率 [MHz]" = instantaneous frequency [MHz]; "(b) 瞬時頻率 finj+θ̇/2π：\n大部分時間逗留在injection側、短暫掃過高頻側" = (b) instantaneous frequency finj+θ̇/2π: it dwells mostly on the injection side, briefly sweeping through the high-frequency side; "注入 finj（頻譜一端的邊緣線）" = injection finj (the edge line at one end of the spectrum); "自由跑 f0（已被拉離）" = free-running f0 (already pulled away); "頻率 [MHz]" / "相對功率 [dB re. max]" = frequency [MHz] / relative power [dB re. max]; "(c) 被拉扯振盪器的頻譜：間距 ωb=√(Δω²-ωL²) 的『單邊』sideband comb（綠色▽＝理論位置 finj+k·ωb/2π；一端貼著注入頻率，低頻側幾乎沒有線）[P4] Eq.(34)" = (c) the spectrum of a pulled oscillator: a "one-sided" sideband comb spaced by ωb=√(Δω²-ωL²) (green ▽ = theoretical positions finj+k·ωb/2π; one end sits at the injection frequency, almost no lines on the low-frequency side) [P4] Eq. (34); suptitle "injection pulling：|Δω| > ωL 鎖不住 → 相位鋸齒滑動 → 不對稱梳狀頻譜（[P3] Sec. V-G + [P4] Eq.(33)-(34)，toy Adler model）" = injection pulling: |Δω| > ωL cannot lock → sawtooth phase slip → an asymmetric comb spectrum ([P3] Sec. V-G + [P4] Eq. (33)-(34), toy Adler model).
...
The site toy $\Gamma(\theta)=\cos\theta+0.3$ ($\alpha=0.3$, DC value $c_0/2=0.3$; **site convention**: here $\alpha$
is the DC offset of the toy ISF, unrelated to [P1]'s NMF $\alpha(\omega_0t)$):
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Legend/axis/title read: "匹配注入 i*∝Γ̃（[P3] Eq.(44)）" = matched injection i*∝Γ̃ ([P3] Eq. (44)); "正弦注入（同一個 Irms，相位對齊）" = sinusoidal injection (same Irms, phase-aligned); "注入相位 x/π [rad/π]" = injection phase x/π [rad/π]; "(a) 同一個 rms 預算的兩種花法（ring 型 ISF, N=…）\n匹配波形把電流全押在 ISF 脈衝上；正弦把大半電流\n花在 Γ̃≈0 的相位（買不到 lock range）" = (a) two ways to spend the same rms budget (ring-type ISF, N=…): the matched waveform puts all its current on the ISF pulse; the sinusoid spends most of its current at phases where Γ̃≈0 (buying no lock range); "匹配注入的 Ω(θ)/2π" = the matched injection's Ω(θ)/2π; "相位差 θ/π [rad/π]" = phase difference θ/π [rad/π]; "(b) lock characteristic（[P3] Eq.(33)）：極值＝lock edge\n匹配注入恰好碰到 Cauchy–Schwarz 上限，增益 ×…" = (b) lock characteristic ([P3] Eq. (33)): the extremum = the lock edge; the matched injection exactly meets the Cauchy-Schwarz bound, gain ×…; suptitle "注入波形設計：固定 Irms 下 lock range 的上限 ωL*=IrmsΓ̃rms（[P3] Eq.(43)–(45)）" = injection-waveform design: the upper bound on lock range at fixed Irms, ωL*=Irms·Γ̃rms ([P3] Eq. (43)-(45)).
