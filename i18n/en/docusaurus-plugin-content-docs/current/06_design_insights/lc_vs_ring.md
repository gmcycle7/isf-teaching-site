...
**Convention flag (owed every time a 2 or 4 appears; corrected in v11)**: the prefactor $8/(3\eta)$ of
Eq.(34) is of the same family as Eq.(23) — both are collected from [P2] Eq.(6), p.792 (Eq.(34) is
$2N\times$Eq.(6)), and the printed Eq.(6) has denominator $8\pi^2f_{off}^2=2\Delta\omega^2$, i.e. this
site's **$\mathcal{L}_{/2}$ family** — **3 dB above** the $/4$ SSB family of [P1] Eq.(21) with its
$4\Delta\omega^2$ (e.g. example B's $-148$ ([P1] Eq.(21), $/4$) vs $-145$ ($/2$), see
[white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise)). So the absolute values
$-82.7$/$-78.0$ dBc/Hz in example 3 below are **$/2$-family** numbers (in the [P1] Eq.(21) bookkeeping
they read $-85.7$/$-81.0$ dBc/Hz); the $\Delta\mathcal{L}$ in example 3 is a **difference** of two
$\mathcal{L}$'s, so the convention factor cancels — **both conventions give the same answer**.
...
square root of the measurement interval ([P2] Eq.(8), p.792; $\kappa$ = square root of Eq.(11), p.793 — the printed Eq.(12) is the time version $\kappa_t=\kappa/\omega_0$):
...
- **This is phase jitter (dimensionless)**: per [P2] Eq.(11)
  ...
  lives in Eq.(10); this site's **phase-version** $\kappa$ contains no $\omega_0$, while the Eq.(12) printed on [P2] p.793 is the
  **time version** $\kappa_t=\kappa/\omega_0$ with Eq.(10) folded in ($\omega_0$ in the denominator, units $\sqrt{\text{s}}$).
...
- $\kappa^2\propto\Gamma_{rms}^2/q_{max}^2\cdot\overline{i_n^2}/\Delta f$ ([P2] Eq.(11)–(12), p.793, re-verified in v11:
  phase version $\kappa=(\Gamma_{rms}/q_{max})\sqrt{(\overline{i_n^2}/\Delta f)/2}$ = square root of Eq.(11); the printed Eq.(12) is $\kappa_t=\kappa/\omega_0$) — the same core
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image (the same script as `lc_vs_ring_isf_comparison.png`; see its Translator's note above/below on this page for the printed terms).
...
## Selection decision table: which oscillator for which job

The steps above compared "whose ISF numbers are better, LC or ring." But the real design question is
usually the other way around — **given an application, which one do you pick?** The answer depends not
only on the VCO's own $\Gamma_{rms}/q_{max}$, but also on **whether a downstream loop filters out the
VCO's close-in noise** — CDR/PLL loop bandwidth is a high-pass filter on VCO phase noise (see
[serdes_clocking_connection](/06_design_insights/serdes_clocking_connection) Step 6). The table below
collects the "who filters, who doesn't, and where the spec is dominated" evidence already derived
elsewhere on this site into one selection table:

| Application | Who filters the VCO (CDR/PLL BW, or none) | Dominant spec region | Choice | ISF rationale |
|---|---|---|---|---|
| **SerDes TX PLL** | Narrow loop BW (filters the reference; VCO close-in appears directly at the output) | close-in 1/f³ and in-band 1/f² | **LC-VCO** | No loop to help — low $\Gamma_{rms}$/high $q_{max}$ is the only way to suppress close-in directly; see [serdes_clocking_connection](/06_design_insights/serdes_clocking_connection) Step 7 |
| **SerDes RX CDR** | Wide loop BW, high-passes away VCO close-in (1/f³ and fast random walk) | white noise left outside the loop / jitter tolerance | **ring-VCO viable** | The loop's high-pass stopband sits right on top of ring's weakness (close-in, fast random walk), and the multiphase outputs come free; see [serdes_clocking_connection](/06_design_insights/serdes_clocking_connection) Steps 6–7 |
| **ADC/DAC sampling clock** | No CDR/PLL relief (free-running sampling, no loop to filter) | close-in integrates straight into SNR (aperture jitter, no high-pass exemption) | **LC + clean reference** | No loop protection — low-offset noise counts in full toward $\sigma_t$; see [adc_aperture_jitter](/06_design_insights/adc_aperture_jitter), [reference_oscillators](/06_design_insights/reference_oscillators) "Connection to SerDes" |
| **RF synthesizer (cellular/Wi-Fi)** | Narrow–moderate loop BW, spec dominated by spot offset | 1 MHz spot $\mathcal{L}$ and reference spurs (2–6 GHz class, order $-120\ldots-135$ dBc/Hz @ 1 MHz, **external rule-of-thumb, not from this site's 5 PDFs, no citation fabricated**) | **LC-VCO** | Spot spec pushes against the FOM ceiling — only the low-$\Gamma_{rms}$/high-$q_{max}$ LC can get in; check feasibility against the [fom_limit](/06_design_insights/fom_limit) ceiling family |
| **Low-power BLE-class** | Power is the priority, but loop relief can't rescue ring's ceiling | $\mathrm{FOM}_{req}$ vs ring ceiling ([P2] Eq.(25) lower bound, $\approx168.3$ dB, see [fom_limit](/06_design_insights/fom_limit) Step 2) | **Still LC** | Ring's $F_{eff}$ ceiling is a hard limit — under N-independence, no power/$N$ tweak buys it back; use the [design_recipe](/06_design_insights/design_recipe) Step 2 ceiling gate to check feasibility |
| **SoC internal digital clock** | Wide loop-BW PLL, tracks out VCO close-in | in-band ps-level timing margin, not close-in dBc/Hz | **ring-VCO** | The wide loop filters away ring's weakness, trading in for small area, wide tuning, and easy digital-process integration |
| **Multiphase / wide tuning-range applications** | System-dependent; this row's choice is unrelated to filtering | number of phases and tuning range, not phase noise | **ring-VCO** | Choosing $N$ should be driven by tuning range/phase count/area, **not** phase noise (single-ended N-independence; this page's Step 2 "design implication") |
| **Instrumentation/radar reference chain** | No CDR; only the $+20\log_{10}N$ of ×N multiplication (no cure) | full-band close-in (Lorentzian linewidth) | **OCXO reference + LC multiplier chain** | The reference's close-in sets the whole chain's floor; multiplier stages still need the low-$\Gamma_{rms}$/high-$q_{max}$ LC; see [clock_chain_budget](/06_design_insights/clock_chain_budget) Rule 1, [reference_oscillators](/06_design_insights/reference_oscillators) |

> **Honesty banner** (following the same rule as [reference_oscillators](/06_design_insights/reference_oscillators) Step 5): any row in the "dominant spec region" column that quotes a concrete dBc/Hz number is an **industry-convention order-of-magnitude range** (external literature, not from this site's 5 PDFs); different vendors/processes/vintages can differ by 10–20 dB, and real selection always goes by the datasheet's $\mathcal{L}(f)$ curve and spur table — this site does not fabricate specific part numbers or paper figures. The other columns (who filters, ISF rationale) are direct applications of the ISF/CDR-high-pass/FOM derivations already verified earlier on this site, not new paper results.

> **One-line summary**: **where a wide loop filters away close-in noise** (RX CDR, SoC-internal PLL) — **ring's weakness stays hidden**; **where no loop filters, or the spec is dominated by close-in/spot noise** (TX PLL, ADC/DAC clocks, RF synthesizers, even power-first low-power designs that still must meet spec, instrumentation reference chains) — **LC's low-$\Gamma_{rms}$/high-$q_{max}$ advantage shows up directly in the spec**.
