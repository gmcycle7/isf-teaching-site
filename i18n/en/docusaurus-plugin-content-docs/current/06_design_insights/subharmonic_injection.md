...
**Site convention**: this $\beta$ is the realignment factor, unrelated to the ISF–APF phase difference $\beta$
of [P4] Eq.(23) (paper_004_large_injection_transient) — the FM modulation index $\beta$ and the MOS
$\beta=\mu C_{ox}W/L$ are likewise distinct symbols, each flagged with a site-convention note at its first use.
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Labels/titles read: "k = N = …：鎖定用的諧波" = k = N = …: the harmonic used for locking; "注入諧波序號 k（頻率 k·fref）" = injected harmonic index k (frequency k·fref); "|Ik| / (2qinj/Tinj)（sinc 包絡）" = |Ik| / (2qinj/Tinj) (sinc envelope); "(a) 矩形脈衝的注入諧波：鎖定要的是第 N 個" = (a) the injection harmonics of a rectangular pulse: locking needs the N-th one; "倍頻比 N（fref=f0/N）" = division ratio N (fref=f0/N); "半 lock range fL [MHz]" = half lock range fL [MHz]; "offset 頻率 f [MHz]" = offset frequency f [MHz]; "Sθ(f) [rad²/Hz]（單邊）" = Sθ(f) [rad²/Hz] (one-sided); "(c) 一階離散迴路的雜訊整形（β = …, N = …）" = (c) the noise shaping of a first-order discrete loop (β = …, N = …); "1/β = … 次注入 ≈ 1/e" = 1/β = … injections ≈ 1/e; "注入次數 k（每次相隔 Tinj=NT0）" = injection count k (each spaced Tinj=NT0 apart); "殘餘相位誤差 |δθk/δθ0|=(1−β)^k" = residual phase error |δθk/δθ0|=(1−β)^k; "(d) 線性化 per-pulse map 的收斂：δθ(k+1)=(1−β)δθk" = (d) convergence of the linearized per-pulse map: δθ(k+1)=(1−β)δθk.
