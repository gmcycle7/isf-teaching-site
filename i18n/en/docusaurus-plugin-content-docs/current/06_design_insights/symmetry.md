...
> **More on the duty-cycle row: a second path through tank $Q$** — a high-$Q$ LC tank filters out the
> higher harmonics of the drive waveform (e.g. the differential-pair current) on its own, pulling the
> output waveform's duty cycle toward 50% and thereby **simultaneously** lowering $c_0$ / the even-order
> $c_n$ ([P1] Sec. IV, end; see the "more linear loads" note in
> [flicker_noise_upconversion](/03_isf_core_theory/flicker_noise_upconversion)). This path is
> **independent of** the other way high $Q$ helps — lowering the white-noise $1/f^2$ floor through
> $\Gamma_{rms}$ (see the $Q$ discussion in [lc_vs_ring](/06_design_insights/lc_vs_ring)). The two
> mechanisms are different — one governs $1/f^3$ (via $c_0$), the other $1/f^2$ (via $\Gamma_{rms}$) —
> but both favor high $Q$. Conversely, a non-50% duty cycle generally makes the even-order $c_n$ larger.
