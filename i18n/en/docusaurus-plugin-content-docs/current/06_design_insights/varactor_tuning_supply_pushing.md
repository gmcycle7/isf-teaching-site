...
> **Prerequisites**: [white_noise_to_phase_noise](/03_isf_core_theory/white_noise_to_phase_noise), [flicker_noise_upconversion](/03_isf_core_theory/flicker_noise_upconversion), [phase_vs_amplitude_noise](/02_foundations/phase_vs_amplitude_noise) | **Next**: [lc_vs_ring](/06_design_insights/lc_vs_ring)

This page assumes you already accept the main thread: "phase has no restoring force → noise integrates through $1/\Delta\omega^2$ into the skirt."
...
The peak phase deviation (FM modulation index) is $\beta=K_{push}V_r/f_m$ (**site convention**: this $\beta$
is the FM modulation index, unrelated to the injection-locking realignment factor $\beta$ on
[subharmonic_injection](/06_design_insights/subharmonic_injection)). **Dimension check**: $\dfrac{(\text{Hz/V})\cdot\text{V}}{\text{Hz}}=$ dimensionless (rad) ✓ — exactly the FM fundamental "frequency deviation ÷ modulation frequency = phase." Plug in the measured value:
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Titles/labels read: "(a) 靜態掃描：f0(VDD)（Level-1 方程級）" = (a) a static sweep: f0(VDD) (equation-level-1); "φ(t) [rad]（去除直流與漂移）" = φ(t) [rad] (DC and drift removed); "(b) 動態：Vr=… mV @ fm=… MHz 漣波 → 相位調變" = (b) dynamic: Vr=… mV @ fm=… MHz ripple → phase modulation; "相對載波 [dBc]" = relative to the carrier [dBc]; "(c) 頻譜：f0±fm 的 FM sidebands" = (c) spectrum: the f0±fm FM sidebands.
