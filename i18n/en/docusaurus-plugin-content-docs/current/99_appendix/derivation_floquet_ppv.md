...
- **That $\omega_0$ is a unit conversion, not new physics**: $v_1$ gives "how many **seconds** per unit $\Delta q$" ($\Delta\alpha$, s); multiplying by $\omega_0$ converts it to "how many **radians**" ($\Delta\phi=\omega_0\Delta\alpha$). This is exactly the same factor this site repeatedly emphasizes in [P2] Eq.(10)–(12), p.793 (the phase variance $\sigma_{\Delta\phi}^2$ of Eq.(11) is converted through Eq.(10) $\sigma_{\Delta\phi}=\omega_0\sigma_{\Delta T}$ into the time-version $\kappa_t$ of the printed Eq.(12), whose denominator **does contain** $\omega_0$; this site's phase version is $\kappa=\omega_0\kappa_t$, re-verified in v11) — "time-domain vs. radian-domain bookkeeping" — and it must be made explicit each time which domain you're working in and when to multiply by $\omega_0$.
...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/titles read: "x（無因次）" / "y=ẋ（無因次）" = x (dimensionless) / y=ẋ (dimensionless); "θ/2π（自 x 上升零交越起算）" = θ/2π (measured from x's rising zero crossing); "Γ/qmax（rad／單位 Δq，正規化）" = Γ/qmax (rad per unit Δq, normalized); "(b) 一次 adjoint 解 → 每個注入軸的 ISF\n歸一化恆定誤差 …，x 軸 rms 差 …" = (b) a single adjoint solve → the ISF for each injection axis; normalized constant error …, x-axis rms difference …; "注入相位 θ/2π（自 x 上升零交越起算）" = injection phase θ/2π (measured from x's rising zero crossing); "(c) PPV = ISF：兩種方法疊在一起\nrms 差 …（vs 脈衝）, …（vs −sin, 歸一）" = (c) PPV = ISF: the two methods overlaid; rms difference … (vs. impulse), … (vs. −sin, normalized).
