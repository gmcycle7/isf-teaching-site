...
> **Translator's note**: this figure is generated by a script with Chinese text baked into the image. Axis labels/titles read: "頻率 frequency （單位：f0；DC=0）" = frequency (units of f0; DC=0); "折疊增益 |c̃k| (HTM gain, 無因次)" = folding gain |c̃k| (HTM gain, dimensionless); "HTM 頻帶折疊：單一輸入 band @ f 被搬到 f+k f0\n每箭頭高度/標籤 =|c̃k|（DC=c0/2，±1=c1/2 …）" = HTM band folding: a single input band @ f is moved to f+k·f0; each arrow's height/label =|c̃k| (DC=c0/2, ±1=c1/2, …); "輸入 band 索引 k" / "輸出 band 索引 m" = input band index k / output band index m; "（沿對角線常數＝只看相差幾個 f0）" = (constant along a diagonal = depends only on how many f0 apart).
